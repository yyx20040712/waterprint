...
| `api/useExportBatch.ts` | 批量导出任务 hook（SVRB D6②：单 body POST→句柄 task_id→GET 兜底→SSE 订阅至终态 outcome[files/failures/error]+列表失效；自建 EventSource 同款复制——features 互不 import 门禁；B5 D3/D4——progress 透传原始 percent+lastOutcome 最近终态[BatchStatusLine 消费源]+sourceRef 覆盖前 close 旧流；SVRB2 PD3~PD7——签名增 projectId[恢复键维度]+cancelActive[activeTaskId 空 no-op+cancelPending 防重；404=终止订阅+清在途+清存储，网络失败=保留可重试]+activeTaskId/cancelError/restoreNotice 三态+sessionStorage 恢复 effect[挂载读存储→GET 快照→fill/resume/drop/keep 四分支；R5 覆盖核对+activeTaskIdRef 取代判定]+终态收束清存储；纯投影面迁 lib/batchTaskView[507>500 预算拆件]透传再导出） |
