...
| `lib/batchTaskStore.ts` | 批量任务恢复存储层（SVRB2 D4：sessionStorage 三函数 read/write/clear——键 waterprint:exportBatch:{projectId}:{kind} 双维度+{taskId,total} 最小载荷+storage 注入面[node 直测 mock]+异常静默降级[token.ts 保守先例——隐私模式/配额满恢复面退化不阻提交]；webapp sessionStorage 首例） |
| `lib/batchTaskStore.test.ts` | 恢复存储 vitest（SVRB2 D8：10 用例——键维度隔离/往返恒等/清除/畸形归 null[非 JSON/taskId 空/total 非正整数]/抛异常 storage 三函数静默/null storage 静默） |
| `lib/restoreDecision.ts` | 恢复决策纯函数层（SVRB2 D5：resolveRestore 四分支[status+终态→fill/非终态→resume/notFound→drop/unreachable→keep——R1 网络异常保留存储]+stillCurrent 覆盖核对[R5——异步窗口防旧任务污染新提交]+isNotFoundApiError 404 分类[UnknownTaskError/HTTP_404]） |
| `lib/restoreDecision.test.ts` | 恢复决策 vitest（SVRB2 D8：11 用例——四分支计划断言/覆盖核对三态/404 分类四边界） |
| `lib/batchTaskView.ts` | 批量任务纯投影层（SVRB2 实现期拆件——useExportBatch 507>500 预算红线触发：三类型[ExportBatchFailure/Outcome/Progress+TaskStatusFace]+六函数[isTerminalTaskState/parseTaskEventData/toBatchOutcome/deriveBatchProgress/makeResumeProgress 恢复首帧/batchStatusText 四态文案]迁此；useExportBatch 透传再导出保公开面） |
...
| `components/ExportButton.tsx` | 导出发起（M5 单元多选[maxTagCount responsive/allowClear 自带形态]+工况选+导出动作三按钮：导出图纸 DXF[N=1 单发/N>1 服务端批量任务——useExportBatch 单 body 提交]/导出模型 IFC[SC1 D7 恒单发]/导出全厂总图 DXF[M5 D5 default 型——ready 闸=工况选定+unit_id 空串=server bare POST]；409 stale 二选一 Modal[force 重发 vs 先重算]+404「先提交计算」引导+501 原文透传+onExported 成功回调[B 批 D5 缝]；B5 D2/D3——三按钮统一 in-flight 门[IFC loading 并入 batching]+进度 toast 内嵌 Progress 行内条[duration=0 持有+终态 destroy 销毁]+挂 BatchStatusLine 常驻回溯行；SVRB2 PD2/PD7——在途行内「取消批量」按钮[activeTaskId 非空即显，提交/恢复两路径同享；loading=cancelPending 防重；TaskPanel 取消先例形态]+cancelError 行内 danger 行+restoreNotice 一次性 info[挂载恢复失效/未成功提示]） |
| `components/BatchStatusLine.tsx` | 批量任务常驻状态行（B5 D3——体验质疑采纳：导出按钮旁「最近一次批量任务」三态回溯[进行中 percent·i/N｜完成 N 项｜失败 kind·unit·原因；取消=已产计数行]；文案派生单源=useExportBatch.batchStatusText 纯函数 node 直测，antd 薄壳不测先例；色调 Typography type 映射） |
...
| `api/useExportBatch.ts` | 批量导出任务 hook（SVRB D6②：单 body POST→句柄 task_id→GET 兜底→SSE 订阅至终态 outcome[files/failures/error]+列表失效；B3-b 订阅经 shared/api/useTaskEventSource.subscribeTaskEvents 命令面〔原自建 EventSource 双实现收敛——features 互不 import 门禁下经 shared 单源；失败计数/超时/取代守卫留本件〕；B5 D3/D4——progress 透传原始 percent+lastOutcome 最近终态[BatchStatusLine 消费源]+sourceRef 覆盖前 close 旧流；SVRB2 PD3~PD7——签名增 projectId[恢复键维度]+cancelActive[activeTaskId 空 no-op+cancelPending 防重；404=终止订阅+清在途+清存储，网络失败=保留可重试]+activeTaskId/cancelError/restoreNotice 三态+sessionStorage 恢复 effect[挂载读存储→GET 快照→fill/resume/drop/keep 四分支；R5 覆盖核对+activeTaskIdRef 取代判定]+终态收束清存储；纯投影面迁 lib/batchTaskView[507>500 预算拆件]透传再导出） |
