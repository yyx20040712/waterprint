...
| `components/ExportButton.tsx` | 导出发起（单元/工况双 Select 两按钮共享+导出动作双按钮[SC1 D7：导出图纸 DXF/导出模型 IFC]：409 stale 二选一 Modal[force 重发 vs 先重算]+404「先提交计算」引导+501 原文透传+onExported 成功回调[B 批 D5 缝]） |
