...
- 预览消费 DXF 的纯投影渲染（禁止前端重建制图逻辑）——**渲染形态已裁决 B=前端渲染库路径（2026-08-30 用户 Ruling）**：dxf-parser/three-dxf 族，新依赖审批与实装归 B 批（立项在册按排期；导出 blob 直接喂解析器零契约改动）；FE9 预览=元数据轻量面（B 批实装前的过渡形态）；
