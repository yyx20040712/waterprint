 ...
 *   冻结不删——本卡为过渡形态记档）：
 *   - DXF 前端渲染库（dxf-parser/three-dxf 族）=新依赖红线（宪法 §2
 *     沿册先例）——**渲染形态已裁决 B=前端渲染库路径（2026-08-30
 *     用户 Ruling）**：新依赖审批与实装归 B 批（立项在册按排期），
 *     本卡维持元数据轻量过渡形态；
...
/** 诚实注记（渲染形态已裁决 B=前端渲染库——实装归 B 批，禁伪装已完成）。 */
const RENDER_NOTE =
  "DXF 线稿渲染未实装——渲染形态已裁决 B=前端渲染库路径（2026-08-30 用户 Ruling；新依赖审批与实装归 B 批）；本卡为元数据轻量预览，图纸内容请经「导出图纸」下载后用 CAD 工具查看。";
