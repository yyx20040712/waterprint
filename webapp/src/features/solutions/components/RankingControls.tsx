 ...
 *
 * 输入:  columns（solutions 响应列名集——选项白名单）+gridFields（B2②
 *        对象载荷——grid 列中文 label 源）+当前 sort 值
 * 输出:  排序下拉（选项=buildSortOptions(columns, gridFields)；切换回调 onChange）
 *
 * 规格说明（FE6 批 6b 段四，D9——替换 M0.5 骨架）：
 *   - 选项=响应 columns（服务端白名单 columns∪{cost}——前端只出 columns
 *     内选项；cost 列现状无列不加，概算注入挂账）；B2（PD10）：grid 列
 *     label 中文化与列头同文案（E 冻结漏报本件消费点——用户裁决
 *     2026-09-09 最小扩容白名单）；
  ...
  return (
    <span>
      ...
      <Select
        ...
        style={{ minWidth: 180 }}
        value={value}
        options={buildSortOptions(columns, gridFields)}
