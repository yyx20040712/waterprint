 ...
 * label_zh=中文真源，列头/sort 选项同文案中文化））：
...
describe("buildTableColumns（D5 动态列模型——响应序直传；B2 grid 列中文列头）", () => {
  ...
  it("golden columns 五列分类：grid→margin→flag→text（构造序=响应序）+grid 列中文列头", () => {
    ...
    expect(byKey.get("n")?.title).toBe("池数（格）"); // 无量纲=无单位后缀
  ...
  it("grid 列头=「label_zh 单位」（单位经 dim 经 shared/dimLabels——PD9）", () => {
    const models = buildTableColumns(
      ["h2", "n"],
      [
    ...
    const byKey = new Map(models.map((m) => [m.key, m]));
    expect(byKey.get("h2")?.title).toBe("有效水深 m"); // 量纲单位后缀
...
describe("buildSortOptions（D9 排序选项=响应 columns 白名单；B2 grid 列 label 中文化同列头）", () => {
  ...
  it("golden columns → 选项序同响应序；grid 列 label=中文列头，非 grid=key", () => {
    expect(
  ...
  it("grid 列 label 与列头同文案（含单位后缀——PD10 防同列两处两语）", () => {
