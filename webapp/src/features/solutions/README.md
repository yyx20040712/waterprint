...
| 文件 | 状态 | 职责 |
|------|------|------|
| `lib/solutionsView.ts` | FE6 实装+C2 重制+V2 批尾 | 纯函数层：SolutionPage 窄化门+动态列模型（两行制 title/unit+固定列名中文；V2：dim 列族中文名=dimFields [manifest.out_dims 声明面] 降级 key）+apply 载荷+排序选项+formatSolutionValue 数值格式化 |
...
| `lib/solutionsFields.ts` | B7 R+ 提取；C2-visual F6 扩+V2 批尾（narrowDimFields） | 枚举结果字段窄化（resultField/grid_fields/dim_fields——V2 计算派生输出量列族）+409 锁冲突判定（isLockConflict/LOCK_HINT）+isUnitEnumerable/enumerateOptions（F6 可枚举判据=manifest grid 非空——不可枚举 disabled 附「无档位参数」后缀；目录未就绪 fail-open）纯函数——solutionsPane 行数预算越界修前进 |
