/**
 * 投影层纯函数测试·图层扩展面（B3 R6 三分自 projectScene.test.ts）：
 * 非默认变换门 L5R/总装红线 L5b/条带 strip L6/Internals 图元选择 FE1 M2/
 * UX2 bounds 聚合/L5R 换轴锚六 describe（node 环境——零 WebGL 依赖）。
 *
 * 输入:  projectScene 纯函数+projectSceneFixtures 夹具（fixture）+
 *        Internals 图元选择面（动态 import 随块整迁——实现前该导出不
 *        存在的红面隔离不殃及全文件）
 * 输出:  图层扩展面契约断言（L5R 单轴收编：rz 换算 Y 轴/rx·ry 拒/scale
 *        仍拒；L5b polyline→boundaries 组+顶点序解码+bounds；L6 strip→
 *        routes 组+角点解码+损坏拒；FE1 M2 dims 键驱动；UX2 全
 *        placements AABB；L5R 保手性 (x,z,−y) 端到端锚——用例逐字随迁
 *        零增减；色值断言零含〔G1-09 先例〕）
 */
import { describe, expect, it } from "vitest";

import { fixture, type FixtureNode } from "./projectSceneFixtures";
import {
