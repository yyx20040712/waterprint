 ...
 *
 * 输入:  BoundaryNode（投影层产出——平面点序 [[x, y], …]，core 压平键
 *        x{i}/y{i} 已在投影层解码；闭合段末点→首点由本层 LineLoop 补
 *        ——core 顶点序即权威）
 * 输出:  R3F 闭合折线（#d4380d——siteplan SiteCanvas COLOR_BOUNDARY 同色
 *        先例字面平行，feature 互不 import 门禁禁跨片引用）
 *
 * 规格说明（L5b 总装模式 2026-09-03）：
 *   - 平面 (x, y) → 世界 (x, 微抬, y)：Y-up 水平面=XZ，红线贴地零高度
  ...
  const geometry = useMemo(
    () =>
      new THREE.BufferGeometry().setFromPoints(
        node.points.map(
          ([x, y]) => new THREE.Vector3(x, BOUNDARY_LIFT_Y, y),
        ),
      ),
    [node.points],
  );
