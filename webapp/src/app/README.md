...
| `canvasPane.tsx` | canvas 标签页装配（C2-thumb 2026-09-11：ThumbnailStage[viewer3d 域]组合穿线——useSceneQuery 同键复用+404/投影拒静默回退象形+切项目清批+unitThumbnails props 注入 CanvasFlow）：design 工艺图 projectFlow 投影→React Flow 只读画布（URL ?project= 单一真相+节点点击选中联动 params）+flex 行满高链（C2-canvas P2） | FE4 实装（FE7 补登）；UX1：S3；C2-canvas 满高；C2-params：侧栏 280 默认+拖拽把手（240~480）+aside flex 列（C2-ALIGN A5r 2026-09-12：底部假设区升位进 ParamTabs 双分页[约束参数\|经验取值]——底部 42% 限高容器退役）；C2-lib（2026-09-10）：libraryFocusId prop 透传（App 持态联动穿线） |；P0-3（2026-09-11）：画布区顶部挂 CanvasEditToolbar（编辑会话/校验/保存/常驻提交计算）+画布区 flex 列化 |
| `canvasEditToolbar.tsx` | 画布编辑工具条（P0-3——task-c2-edit-plan §一.4/一.5）：编辑会话开关⑤[beginEdit 快照/dirty Popconfirm 退场]+校验⑦[警告放行 Alert]+保存[markSaved+invalidate+toast]+常驻提交计算⑥[dirty 先存后算——?task= 回写+TASK_EVENT 六标签联动]；保存体=时点最新 raw+草稿四面（红线⑤双守） | P0-3 实装（2026-09-11）——canvasPane 画布区顶部挂载；E2E-2 批2（2026-09-25）：P0-B runCalc 决策面 decideRunCalc 纯函数化（只读态 body=null 恒放行——修复只读「提交计算」静默 no-op）+rawQuery 加载期按钮 loading+R2-P1-3 参数草稿徽标/诚实 toast（消费 paramsStore.draftHint） |
| `canvasEditToolbar.test.ts` | decideRunCalc 纯函数 vitest（node 环境）——P0-B 决策三态（只读放行/save-run/block-unready） | E2E-2 批2 实装（4 用例绿——以 vitest 实跑为准） |
...
| `jointSolutions.tsx` | 批2d 实装（2026-09-25） | 联合枚举 app 层薄壳（R-B44b-4 兑现）：多选单元（≥2——isUnitEnumerable 过滤）提交条+useRunJointEnumeration 提交；任务态复用面板轨（?task= 写 task 键+TASK_EVENT 派发，不写 enum 键——双轨语义不混）；done 后 narrowJointResult 窄化→JointSolutionsPanel（combos 表+三图 Tabs）；切项目重置 |
| `solutionsNotices.tsx` | 批2d 实装（2026-09-25） | 方案浏览提示组件（自 solutionsPane 抽取——无解诊断/漂移横幅/载荷缺失三提示+取数错误 409/422 回显，纯展示零逻辑变更） |
| `solutionsUrlState.ts` | R2-P2-3 实装（E2E-5 批 2026-09-25；批2d 补登） | ?enum=/?task= URL 键写函数（writeEnumParam/writeTaskParam——replaceState 他键原序保留） |
| `solutionsPane.tsx` | solutions 标签页装配：?project=/?task=/?enum= 三参+单单元枚举提交+TaskPanel（SSE）+方案表/诊断面板+apply 任务轨分立（R1 状态双轨+ENG5 D6 键双轨：表源=enum 键/面板=task 键优先） | FE6 实装（FE6 批记账遗漏，FE7 补登 2026-08-29）；UX1：S3 读方换 useProjectId 订阅；ENG5：枚举提交写 ?enum=（apply 后深链不丢方案表）；R1：表源轨初值 ?enum= 优先、纯 ?task= 旧链兜底（DS-01 修复——FE6 时代分享链表挂载回归）；CP1：ConstraintPicker 挂载（约束勾选→options.constraints 三键载荷——features/params 面组件） |；R2-P2-3（E2E-5 批 2026-09-25）：枚举提交 onSuccess 补 ?task= 回写（深链/刷新任务态恢复不依赖本地存储）
...
| `opsDebugPane.tsx` | 诊断标签页装配（B4-1 实现批 2026-09-19）：?project= 消费+useOpsChainQuery 操作链观测面+ErrorBoundary+空态/404 分级引导+TASK_EVENT 事件桥第七处 invalidate——OpsChainView 时间线+三源聚合卡（诊断摘要/警告计数/trace 统计） | B4-1 实装 |
