...
<!-- 契约头：路径 agent/skills/waterprint/SKILL.md｜职责：教 ZCode 用 WaterPrint 21 工具完成「参数化设计+诊断迭代+说明书生成」｜禁区：本技能不教手算任何工程数值（数值一律出自内核——ADR-019）｜状态：草稿——集成批随实装校对（工具签名按 v2 设计书 D3 权威表誊录，实装后如有出入以实装为准） -->
...
- **数值一律出自内核**：任何工程数值（池容、流量、浓度、造价）都只能来自
