...
> 服务端批量导出任务面（幂等键+SSE 进度）/DWG 转换挂点（ODA 边车，
> 可选）/三维场景与画布 webapp/下载端点（路径安全双闸）/鉴权与 SSE
> 限流（可配置）。**剩余面四类**：服务端批量任务面二期（功能缺口）/
> 纵断真实站距与 L0 独立管底（专业精度，停档待领域专家与用户裁决）/
> 表尺寸 ODA E2E 验收与字体随附（用户承办）/软著申请与开源准备
> （用户目标）——明细见会话工作区 handover 三·bis 对照表。工程面：
