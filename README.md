...
> 常态化（每批 Kimi 一审+deepseek 二审+R 轮处置）。**进行中**：C 批
> 二期逐标签重制（方案表列宽/数值格式化/画布节点象形化+三维缩略图
> /参数面板表单化/三维空旷议题——见 handover §二）。**剩余面**：纵断
> 真实站距与 L0 独立管底（专业精度，停档待裁决）/表尺寸 ODA E2E
> 验收与字体随附（用户承办）/软著申请与开源准备（用户目标）。工程面：
> 门禁 11 项（含 GR-21 弃用到期门禁，TD1）/GR-01~GR-40 治理规则/
> test-lock 263 键只读锁/快照四哈希锚。里程碑：M0 骨架→M1 内核→M2
