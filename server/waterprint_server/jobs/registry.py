"""任务注册表落盘序列化面：记录文档/原子写/恢复扫描（纯函数零状态）。
...
# ══════════════════════════════════════════════════════════════════
# 规格说明（S2 落盘化批 R1 拆分+ENG5 重启语义增强——check_file_budgets
# 门禁 500 行预算「无豁免→拆文件」；行为经 manager 薄壳由 test_manager
# 恢复矩阵用例覆盖）
#
# 【公开接口】
#   TERMINAL_STATES: 终态元组（done/cancelled/failed——manager._TERMINAL
#       同源单定义，消除双定义漂移面）
#   PERSISTABLE_STATES: 可落盘状态全集（queued/running+终态——ENG5 D1
#       submit 初档/running 迁移两非终态时机合法化）
#   INTERRUPTED_ERROR_TYPE: 重启中断诊断名（ENG5 D2——"InterruptedByRestart"）
#   task_document(*, task_id, kind, payload, state, progress, stage,
#       condition_key, stale, error, error_type, result, snapshot_hash,
#       project_id) -> dict：落盘文档（TaskStatus 同构面+snapshot_hash；
...
#      与文件名一致/state∈PERSISTABLE_STATES）；ENG5 D2 非终态记录经
#      mark_interrupted 变换产出（state=failed+error_type=
#      INTERRUPTED_ERROR_TYPE+result=None——运行态任务重启后标 failed
#      可查，裁决②）；损坏越界逐条跳过+structlog warning（fail-visible
#      不阻断启动——单条坏档不炸服务；与锁守卫 fail-closed 场景不同：
...
#   interrupted/幂等表不恢复）；本模块纯函数零 manager 内部类依赖。
#
# 【参照】重写计划 §16 A5/§18；S2 简报 D1~D5；ENG5 简报 D1~D3
# ══════════════════════════════════════════════════════════════════
...
# 可落盘状态全集（ENG5 D1：submit 初档 queued/运行迁移 running 两非终态
# 时机——集外状态落档视为损坏，恢复面跳过）。
PERSISTABLE_STATES: Final[tuple[str, ...]] = ("queued", "running", *TERMINAL_STATES)

# 重启中断诊断名（ENG5 D2 ②类预裁：manager 生命周期事件——不入
# DOMAIN_ERROR_CODES worker 领域异常映射表，error_code=None 诚实）。
INTERRUPTED_ERROR_TYPE: Final[str] = "InterruptedByRestart"


def task_document(  # noqa: PLR0913  # 落盘字段面冻结（13 键平字段——S2 D4；ENG5 更名通用化）
    *,
    task_id: str,
    kind: str,
    ...
    """记录落盘文档（TaskStatus 同构面+snapshot_hash——终态/非终态共用）。

    平字段入参=调用方（manager 薄壳）从 _TaskRecord 投影；subscribers/
    cancel_requested 进程内字段不在入参面（排除语义由签名承载）。
    """
    return {
        "task_id": task_id,
        "kind": kind,
...
def mark_interrupted(document: Mapping[str, Any]) -> dict[str, Any]:
    """恢复面非终态→failed 变换（ENG5 D2——裁决②保守预裁）。

    state=failed+error_type=InterruptedByRestart+result=None（非终态无
    结果面）；error 文案留档重启前状态；progress/stage/snapshot_hash 等
    审计字段原样保留。
    """
    interrupted = dict(document)
    interrupted["state"] = "failed"
    interrupted["error_type"] = INTERRUPTED_ERROR_TYPE
    interrupted["error"] = f"服务重启中断（重启前状态：{document.get('state')}）"
...
def iter_restorable(registry_dir: Path) -> Iterator[tuple[str, Mapping[str, Any]]]:
    """合法记录流（恢复扫描——mtime 升序近似完成时刻序；非终态经变换产出）。

    损坏 JSON/非对象形态/task_id 不一致/状态越界（PERSISTABLE_STATES 外）/
    读取态 IO 异常逐条跳过+structlog warning（fail-visible 不阻断启动）；
    queued/running 记录经 mark_interrupted 变换为 failed 产出（ENG5 D2——
    重启视角终态化视图，manager 零增行消费）。
    """
    ...
    for entry in sorted(registry_dir.glob("*.json"), key=_mtime):
        ...
        try:
            ...
            if state not in PERSISTABLE_STATES:
                raise ValueError(f"状态越界不恢复（合法面 {PERSISTABLE_STATES}）：{state}")
        ...
        if state not in TERMINAL_STATES:
            document = mark_interrupted(document)
