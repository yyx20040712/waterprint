...
def test_aao_pool_slots_declared_l7() -> None:
    """L7：AAO 表行池体声明冻结口径——三槽+剖面双键+29 键 non_drawn（§二
    +曝气头数据面批 n_aerator_raw——n_aerator 归 instance_counts；B4-2a
    能耗五键 q_air/p_blower/e_aeration/p_stir/e_stir）。"""
    ...
    assert frozenset(projection.non_drawn) == frozenset({
        ...
        "q_air", "p_blower", "e_aeration", "p_stir", "e_stir",
    })
