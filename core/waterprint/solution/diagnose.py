...
#       suggestions: tuple[建议, ...]    工业级调节建议（business-logic.md §4）
#   class 建议（可序列化）：
#       param_key（调哪个参数）/ direction（方向）/ magnitude（建议幅度）/
#       basis（依据：条文号或 constraint_kb 键）/ affected_conflicts
#       （预期化解哪个冲突约束）/ expected_effect（一句话影响预判）
#
# 【行为规格】
...
#      ③专家经验表（coefficients，如 BOD5/TN<4 → 越越初沉或外加碳源）。
