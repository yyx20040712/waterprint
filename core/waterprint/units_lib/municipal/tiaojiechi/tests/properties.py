"""municipal_tiaojiechi 物理不变性测试（hypothesis 性质）。
...
#
# 【实装面】
#   - test_active_volume_margin：实际容积≥需调节容积（清单项——TJ-F7
#     圆整裕量恒等 v_act_total≥v_total，随机参数）；
#   - test_actual_retention_margin：实际调节历时≥设计历时（清单项
#     ——v_act_total/q_avg≥t_reg 推论）；
#   - test_volume_monotone_in_t_reg：需调节容积随调节历时单调不减
#     （单调——TJ-F1 t_reg 线性因子，range 五点扫描）；
#   - test_nonneg_finite：全 dims 非负+有限（清单项水位族含内，
#     非负性，随机参数）；
...
from waterprint.units_lib.municipal.tiaojiechi import make_unit, manifest  # noqa: E402

_REPO_ROOT = Path(__file__).resolve().parents[6]
_COEFFS = load_coefficients(_REPO_ROOT / "data" / "coefficients")
_IN_REF = PortRef(unit_id="prop_tjc", port_id="in")
...
def _params(**overrides: float) -> dict[str, float]:
    ...
    for prefix in ("factor.tiaojiechi.", "removal.tiaojiechi.", "factor.screen."):
...
@given(draw=_draws())
@settings(max_examples=40, deadline=None, derandomize=True)
def test_actual_retention_margin(draw: dict[str, float]) -> None:
    """清单项：实际调节历时≥设计历时（v_act_total≥v_total 推论）。"""
    params = _params(**draw)
    dims = _run(params).dims
    assert isinstance(dims, dict)
    assert dims["t_reg_act"] >= params["t_reg"] - 1e-9


def test_volume_monotone_in_t_reg() -> None:
    """单调：需调节容积随调节历时单调不减（TJ-F1 线性因子，五点扫描）。"""
...
def test_nonneg_finite(draw: dict[str, float]) -> None:
    """非负性：一切 dims 量非负且有限（含水位/超高族——水位非负清单项）。"""
    dims = _run(_params(**draw)).dims
    assert isinstance(dims, dict)
    assert dims["h_total"] >= 0.0
