"""污水提升泵房清单声明：参数/端口/去除率引用/条文/工况映射（声明式唯一真源）。
...
# ══════════════════════════════════════════════════════════════════
# 规格说明（结构预留：municipal_wushui_tisheng，内容随 M2 交付）
#
# 【固定形态】UNIT_ID = "municipal_wushui_tisheng"；manifest = load_manifest({...})。
# 【迁移来源】旧系统 mod.json（wushui_tisheng（社区））仅作交叉对照，不作依据——
