"""municipal_cugeshan golden 数值测试（期望值来源：docs/norms/cugeshan.md 签字表）。
...
#   docs/norms/cugeshan.md 算例 1；系数键值逐字取自 data/coefficients
...
#
# 【锁定流程】本文件写完并由人类复核后执行
#   `python scripts/lock_tests.py core/waterprint/units_lib/municipal/cugeshan/tests`
#   转为只读（AGENTS.md §11）。
# ══════════════════════════════════════════════════════════════════
...
from waterprint.units_lib.municipal.cugeshan import make_unit, manifest
...
_IN_REF = PortRef(unit_id="test_cugeshan", port_id="in")
...
def test_manifest_identity() -> None:
    ...
    assert manifest.unit_id == "municipal_cugeshan"
    ...
    assert manifest.removal_refs == {
        "BOD5": "removal.cugeshan.bod5.mod_default",
        "CODCR": "removal.cugeshan.cod.mod_default",
        "SS": "removal.cugeshan.ss.mod_default",
    }
...
def test_main_case_hydraulics() -> None:
    ...
    assert dims["q"] == pytest.approx(0.18775, abs=1e-5)  # CG-F1：0.56325/3
    assert dims["n_gap"] == pytest.approx(6.0)  # CG-F2：5.914 → ceil
    assert dims["B"] == pytest.approx(0.7, abs=1e-9)  # CG-F3：0.64 → 0.7
    assert dims["B1"] == pytest.approx(0.5, abs=1e-9)  # CG-F4：0.44702 → 0.5
    assert dims["v_checked"] == pytest.approx(0.789, abs=1e-3)  # CG-F5：0.7886
    assert dims["v1_checked"] == pytest.approx(0.626, abs=1e-3)  # CG-F6：0.6258
    assert dims["xi"] == pytest.approx(0.1995, abs=1e-4)  # CG-F7：0.19949
    assert dims["h1"] == pytest.approx(0.0183, abs=1e-4)  # CG-F8：0.01832
    assert dims["H"] == pytest.approx(1.0, abs=1e-9)  # CG-F9：0.91832 → 1.0
    assert dims["L"] == pytest.approx(1.8, abs=1e-9)  # CG-F10：1.75457 → 1.8
    assert dims["w_slag"] == pytest.approx(0.6952, abs=1e-4)  # CG-F11：0.695214
    assert dims["mech_clean"] == pytest.approx(1.0)  # CG-F12：0.6952>0.2 机械清渣
    assert dims["ds_slag"] == pytest.approx(139.0, abs=0.05)  # CG-F13：139.0428
    # CG-F14：V_c = L·B·H·n·0.3 = 1.8×0.7×1.0×3×0.3 = 1.134
    assert dims["v_concrete"] == pytest.approx(1.134, abs=1e-6)
...
def test_outflow_passthrough_and_quality() -> None:
    ...
    out_ref = PortRef(unit_id="test_cugeshan", port_id="out")
    ...
    assert pytest.approx(250.0 * (1 - 0.05)) == out_quality.SS
...
def test_velocity_band_warning() -> None:
    """校核带越界：v 带下界（v=0.5 → v_checked≈0.473<0.6）产 WARN。"""
    result = make_unit().compute(_ctx(_params(v=0.5)))
...
def test_formula_ids_registered() -> None:
    ...
    result = make_unit().compute(_ctx(_params()))
    assert result.formula_ids == tuple(f"CG-F{index}" for index in range(1, 15))
