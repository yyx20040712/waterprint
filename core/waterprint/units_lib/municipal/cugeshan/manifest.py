"""粗格栅清单声明：参数/端口/去除率引用/条文/工况映射+公式注册（声明式唯一真源）。

输入:  三表签字真源（docs/norms/cugeshan.md，2026-08-23）+ data/coefficients 0.1.0 键名
输出:  UnitManifest 实例（load_manifest 静态校验通过才算合法）+ CG-F1~F14 公式登记
"""

# ══════════════════════════════════════════════════════════════════
# 规格说明（M1a 实装：M1 先行示范（本批实装）/M2 正式验收）
#
# 【固定形态】UNIT_ID = "municipal_cugeshan"；manifest = load_manifest({...})。
# 【数值真源】参数默认值=三表算例 1 值逐字（n=3/b=65mm/α=75°/h=0.6/
#   v=0.8/v₁=0.7/s=10mm/bar_shape=0/g=9.81/取整步长 0.1）；系数不落本表
#   ——β/k/超高/裕量/W₁/含水率/清渣阈值/流速带/壁厚系数全部经
#   factor.screen.*|factor.cugeshan.* 键消费（app._unit_params 投影，
#   D4）；去除率经 removal.cugeshan.*.mod_default 键（NH3N/TN/TP 依
#   data 包头部注记不建条目）。
# 【公式注册（D2）】CG-F1~F14 逐条 FormulaSpec+register（import 时登记）：
...
#   出处三表 CG-F3/F9/F10）；sin/tan 预处理值以符号传入（sqrt_sin_alpha/
#   sin_alpha/tan_alpha）。
# 【norm_ref 口径】节级引用"GB 50014-2021 §6.3（条文号待核对原文）"，
#   条文级核对挂账（三表条文摘录表在册）；CG-F10 固定段 1.0/0.5/(0.2+h)
#   三表标注"旧系统无出处注释"——挂账同三表。
...
_NORM = "GB 50014-2021 §6.3（条文号待核对原文；docs/norms/cugeshan.md 签字表 2026-08-23）"
...
_FORMULAS: tuple[FormulaSpec, ...] = (
    ...
    FormulaSpec(
        ...
        "w_slag = q_design * 86400 * w1 / (kz * 1000)",
        {
            ...
            "w1": (_D, "栅渣量系数 W1 m3/1e3m3（factor.cugeshan.w1_slag）"),
            "kz": (_D, "总变化系数"),
        },
        _VOL,
        _NORM,
...
manifest = load_manifest(
    {
        "unit_id": UNIT_ID,
        "i18n_key": "units.municipal_cugeshan",
        "version": "1.0",
        "business_line": "municipal",
        # 默认值=三表算例 1 逐字（出处 docs/norms/cugeshan.md 参数行）；
        ...
        "norm_refs": [
            ...
            "docs/norms/cugeshan.md（2026-08-23 领域专家签字手算对照表）",
        ],
        "condition_mappings": [],
