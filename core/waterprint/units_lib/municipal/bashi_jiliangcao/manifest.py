"""巴歇尔计量槽清单声明：参数/端口/去除率引用/条文/工况映射（声明式唯一真源）。
...
# ══════════════════════════════════════════════════════════════════
# 规格说明（结构预留：municipal_bashi_jiliangcao，内容随 M2 交付）
#
# 【固定形态】UNIT_ID = "municipal_bashi_jiliangcao"；manifest = load_manifest({...})。
# 【迁移来源】旧系统 mod.json（bashi_jiliangcao（社区））仅作交叉对照，不作依据——
