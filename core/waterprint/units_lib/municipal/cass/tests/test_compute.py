...
def _params(**overrides: float) -> dict[str, float]:
    ...
    params: dict[str, float] = {
        ...
        "factor.cass.blower.oxygen_transfer_eff": 0.20,
        "factor.cass.blower.pressure_kpa": 70.0,
        "factor.cass.blower.efficiency": 0.70,
        "factor.cass.blower.duty_ratio": 0.50,
...
def test_main_case_energy() -> None:
    """主算例能耗族逐项断言（CA-F29~F33——B4-2a，R-B42a-1 用户批准档）。

    CASS 特化：e_aeration 含 duty_ratio 0.5 周期折算（4h 周期 2h 曝气）；
    p_stir 取选择区容积（CA-F4 v_selector 口径）。"""
    dims = _dims()
    assert dims["q_air"] == pytest.approx(1.871280, abs=1e-6)  # CA-F29
    assert dims["p_blower"] == pytest.approx(187.127976, abs=1e-4)  # CA-F30
    assert dims["e_aeration"] == pytest.approx(2245.535716, abs=1e-3)  # CA-F31
    assert dims["p_stir"] == pytest.approx(6.517631, abs=1e-5)  # CA-F32
    assert dims["e_stir"] == pytest.approx(156.423150, abs=1e-3)  # CA-F33


def test_formula_ids_registered() -> None:
    """formula_ids 非空且全部可在公式注册表解析（§16 A1 漂移防线）。"""
    result = make_unit().compute(_ctx(_params()))
    assert result.formula_ids == tuple(f"CA-F{index}" for index in range(1, 34))
