"""旋流沉砂池单元包根：白名单导出（municipal_chenshachi）。
...
#
# 【身份】UNIT_ID = "municipal_chenshachi"；业务线：市政污水线；
#   旧系统对应 mod：chenshachi（迁移交叉对照，非依据）。
# 【工艺位置】上游：xigeshan 细格栅；下游：chuchenchi 初沉池或
#   tiaojiechi 调节池（按工艺配置）。
# 【实现约定】本包结构由 _template 冻结（AGENTS.md §11）：
#   - 包外只经本 __init__ 白名单访问；禁 import 其他单元包；
#   - 公式 CS-F1~F18 与参数数值真源=docs/norms/chenshachi.md 签字表
...
from waterprint.units_lib.municipal.chenshachi.compute import make_unit
from waterprint.units_lib.municipal.chenshachi.manifest import UNIT_ID, manifest
