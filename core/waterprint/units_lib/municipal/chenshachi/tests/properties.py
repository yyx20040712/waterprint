"""municipal_chenshachi 物理不变性测试（hypothesis 性质）。
...
#
# 【本单元性质清单（旧系统物理测试映射）】
#   停留时间>0、沉砂量≥0（与矿井水平流沉砂各自独立成包）
# 【通用性质】非负性/单调性/守恒（按单元适用）/边界稳定。
#
# 【实装面】
#   - test_retention_positive：实际停留时间 t_actual>0（清单项）；
#   - test_sand_nonneg：沉砂量族 v_sand/q_wet/ds_grit≥0（清单项）；
#   - test_flow_conservation：出流流量=入流（穿流守恒——水量无旁支）；
...
from waterprint.units_lib.municipal.chenshachi import make_unit, manifest  # noqa: E402

_REPO_ROOT = Path(__file__).resolve().parents[6]
_COEFFS = load_coefficients(_REPO_ROOT / "data" / "coefficients")
_IN_REF = PortRef(unit_id="prop_cs", port_id="in")
...
def _params(**overrides: float) -> dict[str, float]:
    ...
    for prefix in ("factor.chenshachi.", "removal.chenshachi.", "factor.screen."):
...
def test_retention_positive(draw: dict[str, float]) -> None:
    """清单项：实际停留时间 t_actual>0（量纲恒正——除式两侧同号）。"""
    dims = _run(_params(**draw)).dims
    assert isinstance(dims, dict)
    assert dims["t_actual"] > 0.0


@given(draw=_draws())
@settings(max_examples=40, deadline=None, derandomize=True)
def test_sand_nonneg(draw: dict[str, float]) -> None:
    """清单项：沉砂量族非负（单池砂斗容积/全厂湿砂量/干砂量）。"""
    dims = _run(_params(**draw)).dims
    assert isinstance(dims, dict)
    assert dims["v_sand"] >= 0.0
    assert dims["q_wet"] >= 0.0
    assert dims["ds_grit"] >= 0.0


@given(draw=_draws())
@settings(max_examples=40, deadline=None, derandomize=True)
def test_flow_conservation(draw: dict[str, float]) -> None:
    """守恒：出流水量=入流水量（旋流沉砂无水量旁支——穿流恒等）。"""
    result = _run(_params(**draw))
    out = result.outflows[PortRef(unit_id="prop_cs", port_id="out")]
...
def test_nonneg_finite(draw: dict[str, float]) -> None:
    """非负性：一切 dims 量非负且有限（几何量/容积量/流量量）。"""
    dims = _run(_params(**draw)).dims
    assert isinstance(dims, dict)
