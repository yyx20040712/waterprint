"""V型滤池单元包根：白名单导出（municipal_vxinglvchi）。
...
#
# 【身份】UNIT_ID = "municipal_vxinglvchi"；业务线：市政污水线；
#   旧系统对应 mod：vxinglvchi（迁移交叉对照，非依据）。
# 【工艺位置】上游：gaomidu 高密沉淀池；下游：ziwai 紫外消毒。
# 【实现约定】本包结构由 _template 冻结（AGENTS.md §11）：
#   - 包外只经本 __init__ 白名单访问；禁 import 其他单元包；
#   - 公式 XL-F1~F19 与参数数值真源=docs/norms/vxinglvchi.md 起草表
...
from waterprint.units_lib.municipal.vxinglvchi.compute import make_unit
from waterprint.units_lib.municipal.vxinglvchi.manifest import UNIT_ID, manifest
