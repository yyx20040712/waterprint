"""municipal_gaomidu 物理不变性测试（hypothesis 性质）。
...
#
# 【本单元性质清单（旧系统物理测试映射）】
#   上升流速≤限值、药剂量≥0（与矿井水高密各自独立成包）
# 【通用性质】非负性/单调性/守恒（按单元适用）/边界稳定。
#
# 【实装面】
#   - test_surface_load_band_warning_coupling：实际液面负荷越带⇔警告
#     在场（清单项——"上升流速≤限值"域面由警告通道诚实呈现）；
#   - test_dose_nonneg：药剂量 m_pac/m_pam≥0（清单项）；
...
from waterprint.units_lib.municipal.gaomidu import make_unit, manifest  # noqa: E402

_REPO_ROOT = Path(__file__).resolve().parents[6]
_COEFFS = load_coefficients(_REPO_ROOT / "data" / "coefficients")
_IN_REF = PortRef(unit_id="prop_gmd", port_id="in")
...
def _params(**overrides: float) -> dict[str, float]:
    ...
    for prefix in ("factor.gaomidu.", "removal.gaomidu.", "factor.screen."):
...
def test_surface_load_band_warning_coupling(draw: dict[str, float]) -> None:
    """清单项：实际液面负荷（上升流速当量）越带⇔警告在场（双向耦合）。"""
    result = _run(_params(**draw))
    dims = result.dims
    assert isinstance(dims, dict)
    band = tuple(_COEFFS.get(key).value for key in _SURFACE_BAND)
    has_warning = any(w.param_key == "q_surface" for w in result.warnings)
    assert has_warning == (not band[0] <= dims["q_surface_act"] <= band[1]), (
        f"q_surface_act={dims['q_surface_act']} 带 {band} 与警告在场 {has_warning} 失耦"
    )
...
def test_sludge_ds_conservation(draw: dict[str, float]) -> None:
    ...
    result = _run(_params(**draw))
    out_quality = result.outqualities[PortRef(unit_id="prop_gmd", port_id="out")]
    sludge = result.outflows[PortRef(unit_id="prop_gmd", port_id="sludge_out")]
