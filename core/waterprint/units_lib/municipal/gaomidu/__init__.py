"""高密沉淀池单元包根：白名单导出（municipal_gaomidu）。
...
#
# 【身份】UNIT_ID = "municipal_gaomidu"；业务线：市政污水线；
#   旧系统对应 mod：gaomidu（迁移交叉对照，非依据）。
# 【工艺位置】上游：erchunchi 二沉池（深度处理段首单元）；下游：
#   vxinglvchi V 型滤池。
# 【实现约定】本包结构由 _template 冻结（AGENTS.md §11）：
#   - 包外只经本 __init__ 白名单访问；禁 import 其他单元包；
#   - 公式 GM-F1~F20 与参数数值真源=docs/norms/gaomidu.md 起草表
...
from waterprint.units_lib.municipal.gaomidu.compute import make_unit
from waterprint.units_lib.municipal.gaomidu.manifest import UNIT_ID, manifest
