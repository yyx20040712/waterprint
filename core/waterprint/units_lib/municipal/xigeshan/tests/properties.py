"""municipal_xigeshan 物理不变性测试（hypothesis 性质）。
...
from waterprint.units_lib.municipal.xigeshan import make_unit, manifest  # noqa: E402

_REPO_ROOT = Path(__file__).resolve().parents[6]
_COEFFS = load_coefficients(_REPO_ROOT / "data" / "coefficients")
_IN_REF = PortRef(unit_id="prop_xigeshan", port_id="in")
...
def _params(**overrides: float) -> dict[str, float]:
    ...
    for prefix in ("factor.xigeshan.", "removal.xigeshan.", "factor.screen."):
