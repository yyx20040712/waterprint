"""细格栅计算实现：唯一计算源（XG-F1~F14 全经 registry.apply 求值）。
...
def _geometry(ctx: UnitContext, p: dict[str, float], flow: WaterFlow) -> dict[str, float]:
    ...
    q = _apply(ctx, "XG-F1", {"q_design": flow.q_design, "n": p["n"]})
    n_gap = float(
        math.ceil(
            _apply(
                ctx,
    ...
    b1_width = _ceil_step(
        _apply(ctx, "XG-F4", {"q": q, "h": p["h"], "v1": p["v1"]}),
        step,
        ctx.unit_id,
    )
    return {
        ...
        "v1_checked": _apply(ctx, "XG-F6", {"q": q, "h": p["h"], "b1": b1_width}),
...
def _losses(
    ...
    w_slag = _apply(
        ...
        "XG-F11",
        {
            ...
            "w1": _factor(p, "factor.xigeshan.w1_slag", unit_id),
            "kz": flow.kz,
        },
    )
    mech_margin = _apply(
