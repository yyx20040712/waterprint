"""紫外消毒单元包根：白名单导出（municipal_ziwai）。
...
#
# 【身份】UNIT_ID = "municipal_ziwai"；业务线：市政污水线；
#   旧系统对应 mod：ziwai（迁移交叉对照，非依据）。
# 【工艺位置】上游：vxinglvchi V 型滤池；下游：bashi_jiliangcao
#   巴歇尔计量槽（全厂终水出流段）。
# 【实现约定】本包结构由 _template 冻结（AGENTS.md §11）：
#   - 包外只经本 __init__ 白名单访问；禁 import 其他单元包；
#   - 公式 ZW-F1~F13 与参数数值真源=docs/norms/ziwai.md 起草表
...
from waterprint.units_lib.municipal.ziwai.compute import make_unit
from waterprint.units_lib.municipal.ziwai.manifest import UNIT_ID, manifest
