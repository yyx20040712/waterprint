...
#
# 【实装面】
#   - test_seed_recovery_domain：磁回收率∈(0,1]（清单项——数据包真源
#     factor.mine_cifenli.seed.recovery 域守卫）；
#   - test_seed_net_identity_monotone：磁种净耗=投加量×未回收比
#     （KS-F8 恒等）且随投加量单调不减（清单项"随投加量单调"的实装
#     面——出水质去除率系数据包 mod_default 固定档，投加量响应面在
#     磁种净耗侧，诚实注记）；
#   - test_sludge_ds_conservation：磁泥产股 DS=SS 去除量×平均流量
#     （守恒——固流恒等）；
...
from waterprint.units_lib.mine_water.cifenli import make_unit, manifest  # noqa: E402

_REPO_ROOT = Path(__file__).resolve().parents[6]
_COEFFS = load_coefficients(_REPO_ROOT / "data" / "coefficients")
_IN_REF = PortRef(unit_id="prop_mcf", port_id="in")
...
_RECOVERY = "factor.mine_cifenli.seed.recovery"
...
def _params(**overrides: float) -> dict[str, float]:
    ...
    for prefix in ("factor.mine_cifenli.", "removal.mine_cifenli.", "factor.screen."):
...
def test_seed_recovery_domain() -> None:
    """清单项：磁回收率∈(0,1]（数据包真源域守卫）。"""
    recovery = _COEFFS.get(_RECOVERY).value
    assert 0.0 < recovery <= 1.0, f"{_RECOVERY}={recovery} 越域 (0,1]"


@given(draw=_draws())
@settings(max_examples=40, deadline=None, derandomize=True)
def test_seed_net_identity_monotone(draw: dict[str, float]) -> None:
    """清单项：磁种净耗=投加量×未回收比（KS-F8 恒等）且≥0。"""
    params = _params(**draw)
    dims = _run(params).dims
    assert isinstance(dims, dict)
    recovery = _COEFFS.get(_RECOVERY).value
    assert dims["m_seed_net"] == pytest.approx(
        params["m_seed"] * (1.0 - recovery), rel=1e-9, abs=1e-12
    )
    assert dims["m_seed_net"] >= 0.0


def test_seed_net_monotone_in_dose() -> None:
    """单调：磁种净耗随投加量 m_seed 单调不减（五点扫描）。"""
    spec = next(s for s in manifest.params if s.field_id == "m_seed")
    base = spec.default
    points = [base * factor for factor in (0.5, 0.75, 1.0, 1.5, 2.0)]
    previous: float | None = None
    for m_seed in points:
        dims = _run(_params(m_seed=m_seed)).dims
        assert isinstance(dims, dict)
        current = dims["m_seed_net"]
        if previous is not None:
            assert current >= previous - 1e-12, (
                f"m_seed={m_seed}: m_seed_net={current} < 前点 {previous}（单调性破坏）"
            )
        previous = current
...
def test_sludge_ds_conservation(draw: dict[str, float]) -> None:
    """守恒：磁泥产股 DS=SS 去除量（(SS_in−SS_out)×q_avg 恒等）。"""
    result = _run(_params(**draw))
    out_quality = result.outqualities[PortRef(unit_id="prop_mcf", port_id="out")]
    sludge = result.outflows[PortRef(unit_id="prop_mcf", port_id="sludge_out")]
    ...
    assert sludge.ds == pytest.approx(removed_load, rel=1e-9, abs=1e-12), (
        f"磁泥 DS={sludge.ds} ≠ SS 去除负荷 {removed_load}（固流守恒破坏）"
    )
