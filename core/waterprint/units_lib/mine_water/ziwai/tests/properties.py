"""mine_water_ziwai 物理不变性测试（hypothesis 性质）。
...
#
# 【实装面】
#   - test_dose_identity：实配剂量=灯管排数×单排设计剂量（清单项
#     ——KZ-F7/F8 累积恒等 dose_act=n_rows×dose_row≥设计剂量行值，
#     随机参数）；
...
from waterprint.units_lib.mine_water.ziwai import make_unit, manifest  # noqa: E402

_REPO_ROOT = Path(__file__).resolve().parents[6]
_COEFFS = load_coefficients(_REPO_ROOT / "data" / "coefficients")
_IN_REF = PortRef(unit_id="prop_mzw", port_id="in")
...
_DOSE_DESIGN = "factor.mine_ziwai.dose"
...
def _params(**overrides: float) -> dict[str, float]:
    ...
    for prefix in ("factor.mine_ziwai.", "removal.mine_ziwai.", "factor.screen."):
...
def test_dose_identity(draw: dict[str, float]) -> None:
    """清单项：实配剂量=排数×单排剂量且≥设计剂量（KZ-F7/F8 累积恒等）。"""
    dims = _run(_params(**draw)).dims
    assert isinstance(dims, dict)
    assert dims["dose_act"] == pytest.approx(
        dims["n_rows"] * dims["dose_row"], rel=1e-9
    )
    design = _COEFFS.get(_DOSE_DESIGN).value
    assert dims["dose_act"] >= design - 1e-9, (
        f"dose_act={dims['dose_act']} < 设计剂量 {design}（清单项越下限）"
    )
...
def test_flow_conservation(draw: dict[str, float]) -> None:
    """守恒：出流水量=入流水量（紫外渠穿流恒等）。"""
    result = _run(_params(**draw))
    out = result.outflows[PortRef(unit_id="prop_mzw", port_id="out")]
...
def test_nonneg_finite(draw: dict[str, float]) -> None:
    """非负性：一切 dims 量非负且有限（渠道几何/剂量/接触面）。"""
    dims = _run(_params(**draw)).dims
    assert isinstance(dims, dict)
