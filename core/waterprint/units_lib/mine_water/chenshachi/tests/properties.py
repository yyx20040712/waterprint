"""mine_water_chenshachi 物理不变性测试（hypothesis 性质）。
...
#
# 【本单元性质清单（旧系统物理测试映射）】
#   水平流速在设计带内（与市政旋流沉砂各自独立成包）
# 【通用性质】非负性/单调性/守恒（按单元适用）/边界稳定。
#
# 【实装面】
#   - test_velocity_band_warning_coupling：实际水平流速越带⇔警告在场
#     （清单项——"在设计带内"域面由警告通道诚实呈现）；
#   - test_flow_conservation：出流水量=入流（穿流守恒）；
#   - test_sand_nonneg：沉砂/砂斗量族非负（通用非负面清单项）；
...
from waterprint.units_lib.mine_water.chenshachi import make_unit, manifest  # noqa: E402

_REPO_ROOT = Path(__file__).resolve().parents[6]
_COEFFS = load_coefficients(_REPO_ROOT / "data" / "coefficients")
_IN_REF = PortRef(unit_id="prop_mcs", port_id="in")
...
def _params(**overrides: float) -> dict[str, float]:
    ...
    for prefix in (
        "factor.mine_chenshachi.",
        "removal.mine_chenshachi.",
        "factor.screen.",
    ):
...
def test_velocity_band_warning_coupling(draw: dict[str, float]) -> None:
    """清单项：实际水平流速越带⇔警告在场（双向耦合）。"""
    result = _run(_params(**draw))
    dims = result.dims
    assert isinstance(dims, dict)
    band = tuple(_COEFFS.get(key).value for key in _VEL_BAND)
    has_warning = any(w.param_key == "v_h" for w in result.warnings)
    assert has_warning == (not band[0] <= dims["v_h_act"] <= band[1]), (
        f"v_h_act={dims['v_h_act']} 带 {band} 与警告在场 {has_warning} 失耦"
    )
...
def test_flow_conservation(draw: dict[str, float]) -> None:
    """守恒：出流水量=入流水量（平流沉砂穿流恒等）。"""
    result = _run(_params(**draw))
    out = result.outflows[PortRef(unit_id="prop_mcs", port_id="out")]
...
def test_nonneg_finite(draw: dict[str, float]) -> None:
    """非负性：一切 dims 量非负且有限（含沉砂/砂斗/产股面）。"""
    dims = _run(_params(**draw)).dims
    assert isinstance(dims, dict)
    assert dims["v_sand"] >= 0.0
    assert dims["v_hopper"] >= 0.0
