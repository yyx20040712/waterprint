...
#
# 【实装面】
#   - test_flow_and_quality_passthrough：出流水量与水质=入流（清单项
#     ——穿流结构性恒等，随机参数）；
#   - test_volume_nonneg：井容/有效容积≥0（清单项）；
#   - test_retention_nonneg：实际停留时间 t_act≥0（清单项）；
...
from waterprint.units_lib.conveyance.jishuijing import make_unit, manifest  # noqa: E402

_REPO_ROOT = Path(__file__).resolve().parents[6]
_COEFFS = load_coefficients(_REPO_ROOT / "data" / "coefficients")
_IN_REF = PortRef(unit_id="prop_jsj", port_id="in")
...
def _params(**overrides: float) -> dict[str, float]:
    ...
    for prefix in ("factor.jishuijing.", "removal.jishuijing.", "factor.screen."):
...
def test_flow_and_quality_passthrough(draw: dict[str, float]) -> None:
    """清单项守恒：出流水量=入流且六指标=入流（穿流结构性恒等）。"""
    result = _run(_params(**draw))
    out = result.outflows[PortRef(unit_id="prop_jsj", port_id="out")]
    ...
    out_quality = result.outqualities[PortRef(unit_id="prop_jsj", port_id="out")]
    for indicator in ("BOD5", "CODCR", "SS", "NH3N", "TN", "TP"):
        assert float(getattr(out_quality, indicator)) == pytest.approx(
            float(getattr(_QUALITY, indicator)), rel=1e-12
        )


@given(draw=_draws())
@settings(max_examples=40, deadline=None, derandomize=True)
def test_volume_nonneg(draw: dict[str, float]) -> None:
    """清单项：井容 v_wel≥0、v_concrete≥0（容积物理域）。"""
    dims = _run(_params(**draw)).dims
    assert isinstance(dims, dict)
    assert dims["v_well"] >= 0.0
    assert dims["v_concrete"] >= 0.0


@given(draw=_draws())
@settings(max_examples=40, deadline=None, derandomize=True)
def test_retention_nonneg(draw: dict[str, float]) -> None:
    """清单项：实际停留时间 t_act≥0。"""
    dims = _run(_params(**draw)).dims
    assert isinstance(dims, dict)
    assert dims["t_act"] >= 0.0


@given(draw=_draws())
@settings(max_examples=40, deadline=None, derandomize=True)
def test_nonneg_finite(draw: dict[str, float]) -> None:
    """非负性：一切 dims 量非负且有限（井径/容积/流量面）。"""
    dims = _run(_params(**draw)).dims
    assert isinstance(dims, dict)
