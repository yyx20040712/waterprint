...
#     分流=Σ出流恒等，随机参数含池数档位）；
#   - test_water_level_continuity：井总深≥有效水深（清单项——
#     水位衔接裕量恒等 h_total≥h_well，无跌水突变的结构前提）；
#   - test_per_series_flow_monotone_in_pools：单系列流量随池数 n
#     档位单调不增（单调——q/n 均分恒等）；
...
from waterprint.units_lib.conveyance.jipeishuijing import (  # noqa: E402
    make_unit,
    manifest,
)
...
_IN_REF = PortRef(unit_id="prop_jpsj", port_id="in")
...
def _params(**overrides: float) -> dict[str, float]:
    ...
    for prefix in (
        "factor.jipeishuijing.",
        "removal.jipeishuijing.",
        "factor.screen.",
    ):
...
def test_split_flow_conservation(draw: dict[str, float]) -> None:
    """清单项守恒：Σ出流口水量=入流（分流=Σ出流恒等）。"""
    params = _params(**draw)
    result = _run(params)
    count = int(params["n"])
    total = 0.0
    for index in range(1, count + 1):
        out = result.outflows[PortRef(unit_id="prop_jpsj", port_id=f"out_{index}")]
    ...
    assert total == pytest.approx(_FLOW.q_avg_daily, rel=1e-9), (
        f"Σ出流={total} ≠ 入流={_FLOW.q_avg_daily}（分流守恒破坏）"
    )


@given(draw=_draws())
@settings(max_examples=40, deadline=None, derandomize=True)
def test_water_level_continuity(draw: dict[str, float]) -> None:
    """清单项：井总深≥有效水深（水位衔接裕量——无跌水突变前提）。"""
    params = _params(**draw)
    dims = _run(params).dims
    assert isinstance(dims, dict)
    assert dims["h_total"] >= params["h_well"] - 1e-9


def test_per_series_flow_monotone_in_pools() -> None:
    """单调：单系列流量随池数 n 档位单调不增（q/n 均分恒等）。"""
    ...
    for n in grid:
        ...
        assert isinstance(dims, dict)
        current = dims["q_series"]
        if previous is not None:
            assert current <= previous + 1e-12, (
                f"n={n}: q_series={current} > 前档 {previous}（单调性破坏）"
            )
        previous = current
