"""集配水井清单声明：参数/端口/去除率引用/条文/工况映射（声明式唯一真源）。

输入:  本单元工程定义（范围与出处见 docs/norms，M3 交付期冻结）
输出:  UnitManifest 实例（load_manifest 静态校验通过才算合法）
"""

# ══════════════════════════════════════════════════════════════════
# 规格说明（结构预留：conveyance_jipeishuijing，内容随 M3 交付）
#
# 【固定形态】UNIT_ID = "conveyance_jipeishuijing"；manifest = load_manifest({...})。
# 【迁移来源】旧系统 mod.json（jipeishuijing）仅作交叉对照，不作依据——
