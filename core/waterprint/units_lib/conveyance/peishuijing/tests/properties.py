...
#
# 【实装面】
#   - test_allocation_sum_conservation：Σ支路口水量=入流（清单项——
#     分配和=总流量恒等，随机参数含配水数档位）；
#   - test_branch_flow_nonneg：各支路流量非负（清单项）；
#   - test_per_branch_flow_monotone_in_count：单支路流量随配水数 n
#     档位单调不增（单调——q/n 均分恒等）；
...
from waterprint.units_lib.conveyance.peishuijing import make_unit, manifest  # noqa: E402

_REPO_ROOT = Path(__file__).resolve().parents[6]
_COEFFS = load_coefficients(_REPO_ROOT / "data" / "coefficients")
_IN_REF = PortRef(unit_id="prop_psj", port_id="in")
...
def _params(**overrides: float) -> dict[str, float]:
    ...
    for prefix in ("factor.peishuijing.", "removal.peishuijing.", "factor.screen."):
...
def test_allocation_sum_conservation(draw: dict[str, float]) -> None:
    """清单项守恒：Σ支路口水量=入流（分配和=总流量恒等）+支路非负。"""
    params = _params(**draw)
    result = _run(params)
    count = int(params["n"])
    total = 0.0
    for index in range(1, count + 1):
        out = result.outflows[PortRef(unit_id="prop_psj", port_id=f"out_{index}")]
        assert isinstance(out, WaterFlow)
        assert out.q_avg_daily >= 0.0, f"支路 out_{index} 流量为负"
        total += out.q_avg_daily
    assert total == pytest.approx(_FLOW.q_avg_daily, rel=1e-9), (
        f"Σ支路={total} ≠ 入流={_FLOW.q_avg_daily}（分配守恒破坏）"
    )


def test_per_branch_flow_monotone_in_count() -> None:
    """单调：单支路流量随配水数 n 档位单调不增（q/n 均分恒等）。"""
    ...
    for n in grid:
        ...
        if previous is not None:
            assert current <= previous + 1e-12, (
                f"n={n}: q_each={current} > 前档 {previous}（单调性破坏）"
            )
        previous = current
