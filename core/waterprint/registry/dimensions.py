...
# ── B3 R3 数据登记面外迁（2026-09-05）：pool_length+八批次组纯五元组序列
#    迁 dimension_specs.py（零 import dimensions 环免疫；批次组注释随迁）；
#    本文件构造循环 FieldSpec(*tup) 机械转写后登记——登记守卫=转写安全网
#    （错数据必在 import 期显式红，与现状同）；bind_dimension_lookup 统一
#    置于全部登记后（原 bind 在 M1A 组后非文件尾——搬运后 import 副作用
#    终态恒等：登记全集先齐、查询钩子后装，bind-once 语义逐字保）。 ──
for _tuple in DIMENSION_SPECS:
    register_dimension(FieldSpec(*_tuple))
