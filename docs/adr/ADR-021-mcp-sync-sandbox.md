...
| D4 | **会话追溯链**：sessions/{session_id}.jsonl 逐事件（指令/工具调用/参数 patch 改前改后与被拒条目）→design_digest→计算迹→产物 .meta.json 八键，逐字段可对账；脱敏三条款（沙箱内相对化/正式区哈希/env 值掩蔽）——先路径后环境变量的顺序铁律（反序泄漏盘符前缀，实测复现） | 与既有 repro 三元组衔接成完整链条"AI 会话→参数→计算迹→产物" |
