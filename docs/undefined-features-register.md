...
| UF-08 | 引擎 | 引擎技术参数落点：loop 阻尼/容差、LRU 512 条/512MB 等无合规去处——assumptions 要求规范出处（给不出）vs 代码字面量撞魔法数字门禁 | 已定义→RunEnv.engine_params（T4 D3 冻结，commit：97ae1f9）：以带调节影响元数据的引擎默认条目入 engine_params 字段（run_env.py R2/app 装配 T7）；数值 T7 executor 实现期冻结，禁散落代码字面量（GR-15）。**已实现·T7a（2026-08-25）**：类型面 contracts/run_env.py——EngineParam（value/source/note，GR-15 出处门槛）+engine_params 字段（commit 4fa8359）；数值面 registry/assumptions loop.* 三条条目——tolerance=1e-10/max_iterations=200/damping=0.8，source/note/tuning_impact 俱全、工程惯例类出处（commit c254292）；app 装配从 DEFAULT_ASSUMPTIONS 提取 loop.* 三键投影 EngineParam 归 T7b（run_env 规格 R2 投影口径已注记）；LRU 512 条/512MB 缓存参数不在本批（缓存属 incremental 优化层，随其实现任务冻结） | unified B5 / GLM-01 |
