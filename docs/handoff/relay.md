...
- next_batch: 批 2 拟定者任务书起草+派发（备源承载——《裁决书》三轮裁决③）
...
- **手动会话按板领批=当前唯一路径**（本项目未部署自动化接力；未来若立火式
  ...
  文档批轻量双审）→ **收口四步=勾选框更新+批次日志追加（勿改写）+
  收口即推送+守望 CI 至绿**（三轮裁决④常态化）。
- 领批冲突：无 claim 字段（单会话作业）——多会话并行时先建 claim 机制再开工。
...
- 纵断真实站距/L0 独立管底（停档待裁决）｜ODA E2E（用户承办）｜软著签章页
  （用户暂缓）｜UF 开放 16 条按册｜sunset 观察中 3 条（≥10 批零触发后复核）
