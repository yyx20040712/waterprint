...
- 首发作废后修正版重发：新建任务视图三次未开（侧边栏位按钮点击无效/顶栏同名牌无效/切换侧边栏点击遭 a11y 树重编号竞态否决——本调度会话自身转录实时增长使共享窗口树持续重编号，观察→动作窗口内恒漂移）。按前台保护规范停止重试。呈报用户三径：①用户亲开新任务（选择项目=智水蓝图+模型=GLM-5.3）后告知，调度员注入指令（键入通道此前实证可靠）；②用户直接粘贴指令文本自发送；③火班每 10min 自动重试（同条件可能续败）。修正版发布成立时更新 last_dispatch_utc。
