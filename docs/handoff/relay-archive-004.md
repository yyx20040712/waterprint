# relay-archive-004（批次日志滚动归档·四号档）

> 增补十二 2026-09-25 立档：流程修复批（rev2 升级+迁火）随批滚动——主板「批次日志区全量」
> 原文零改动纯迁移（003 号档同款纪律）。含换防归档件（原 protocol 段）、增补十/十一、
> 欠账一/二及 B4-2a 时代以来全部批次史。
...
- 用户实时指出首发任务两缺陷：①「选择项目」未生效——工作目录未绑定智水蓝图（调度员 UI 点菜单项后未回读确认即 Escape 收菜单）；②模型未跟随当前会话模型（当班会话模型）——开批流程缺模型设置步。**两项入开批守则：发送前验证清单=①项目按钮标签回读=智水蓝图②模型回读=跟随当前会话③指令文本回读落框**。
...
### 欠账二 — 2026-09-24T18:12Z（修正版重发 UI 通道连续失败——树重编号竞态系统性；呈报用户协助路径）
- 首发作废后修正版重发：新建任务视图三次未开（侧边栏位按钮点击无效/顶栏同名牌无效/切换侧边栏点击遭 a11y 树重编号竞态否决——本调度会话自身转录实时增长使共享窗口树持续重编号，观察→动作窗口内恒漂移）。按前台保护规范停止重试。呈报用户三径：①用户亲开新任务（选择项目=智水蓝图+模型=当班会话模型）后告知，调度员注入指令（键入通道此前实证可靠）；②用户直接粘贴指令文本自发送；③火班每 10min 自动重试（同条件可能续败）。修正版发布成立时更新 last_dispatch_utc。
- 附：推送欠账持续（本地代理断——3eb5550ec6/e74cc18f8d/26c374bb7a 三笔待推，代理恢复即推）。
