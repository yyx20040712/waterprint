> 批次日志滚动归档件 001（B2-6 收口起立档）——真相史延续载体，追加勿改写
> 纪律同 relay.md 主板：只追加不回改；本件收录 2026-09-18 建板~B2-4 增补二段
> （batch 0~3 全量+B2-1~B2-4 全量+调度员增补一/二），自 relay.md 原文零改动
> 纯迁移（500 行全局预算与状态板追加制长跑冲突的滚动处置，batch 2 README
> 编年史迁档同款先例）；主板上界满 400 行再滚下一档（-002 续档）。
...
> 〔B4-1 批滚动迁入 2026-09-19〕以下为调度员增补三~batch B3-a 补记段（2026-09-19），
> 自 relay.md 原文零改动纯迁移（主板 503 行超 500 全局预算——B2-6 立档同款滚动处置）。
