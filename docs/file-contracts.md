...
| `server/waterprint_server/jobs/worker.py` | 进程池入口（run_task 三参 pickle 边界唯一面/kind 映射表集中一处经 app[UF-33]/RunEnv 协议适配器[UF-46]/阶段取消轮询[UF-49]/feather+serialize 原子落盘/导入零副作用；R2-C：export_batch dxf 项双产物挂钩〔sidecars 二道闸 K-03+转换前取消 K-02+D-02 后缀闸入 _safe_out_name〕；SVRB：批首 _load_project[project_path 通道——calc 同款]+逐项 _build_drawing_kwargs[与单产物等价]+ifc 放行三件[_EXPORT_KINDS/ifc 后缀特判/ifc 边车]+部分失败协议[逐项 try/except 收集 failures——部分失败=done+failures/全失败=raise 聚合]+stage 带 unit 段，500 行） | payload+令牌 | 结果+进度 |
