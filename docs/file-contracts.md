...
| `server/waterprint_server/routers/calc.py` | 计算/枚举八端点（幂等/取消/分页+FD design-map 同步直返[asyncio.to_thread 承载防事件循环阻塞——计算面首例；axes 长度 1~2=pydantic 422 面]+P2 次批 GET trust 可信度报告[ADR-012 D8 2026-09-12]，215 行） | 任务请求+轴声明 | 任务句柄/TaskStatus/SolutionPage/DesignMapResponse |
