...
| `core/waterprint/contracts/manifest.py` | L0 | 模组清单 schema 正门（加载即静态校验：R1a~R1e/R4；校验器机器部分 T4 拆至 manifest_validation.py；ParamSpec B2 起含可选 label_zh——参数级中文名真源在各单元 manifest 数据；V2 GOV5 批尾起 UnitManifest +out_dims 可选顶层键——输出量声明面 [OutDimSpec {field_id,dim,label_zh}——dims 键=计算派生输出量，R1a 登记守卫不适用仅 DimKey 枚举校验；方案表 dim 列中文化真源，AAO 首例 21 条]；GR-36 类①冻结 schema） | 清单数据 | ParamSpec（含 label_zh 可选键）、OutDimSpec（V2）、ConditionMapping、UnitManifest（含 out_dims）、load_manifest、bind_dimension_lookup、InvalidUnitConfig（后两者为 manifest_validation 再导出） |
| `core/waterprint/contracts/manifest_validation.py` | L0 | manifest 冻结 schema 的静态校验器集 + 装配槽（GR-36 类①；T4 拆分自 manifest.py 纯移动，bind_dimension_lookup 单槽 bind-once；_PARAM_KEYS B2 起六键含 label_zh） | 清单数据节点 | 守卫器/键集常量（_PARAM_KEYS 六键）/InvalidUnitConfig、bind_dimension_lookup（经 manifest.py 再导出） |
...
| `core/waterprint/contracts/trust.py` | L0 | 结果可信度诊断契约（P2 次批 ADR-012 D1——独立并列 artifact 数据面：收敛/水量闭合/裕度；确定性序列化纪律同 result_schema；iterations int 归一往返前提） | executor 采集+app 纯投影 | DiagnosticsReport、LoopRunStats、FlowClosure、ClosureLine、UnitImbalance、IndicatorMargin、DiagSink（协议）、serialize_diag、deserialize_diag、InvalidDiagnosticsError |
...
| `core/waterprint/contracts/edge_parsing.py` | L0 | design.edges 解析内核（B3-c 批 2c 收敛件 2026-09-19：《裁决书》方案二 2c——app_assembly._endpoint/_edges 与 executor_assembly._endpoint/_edges_from_design 双胞胎校验/消息/遍历结构单源下沉；拒绝异常类经 error 参数注入[批 3a latest_calc not_found 先例同型——400/422 契约面经类型注入恒等]；消息统一含「得到」版[app 侧文本零变=validate_design_structure 汇总面零连带；executor 侧两处补分隔词，定案 J3 呈案 A]；GR-36 类③受限内核，expr.py 先例） | design.edges 原始元素+error 异常类 | endpoint_from、edges_from（经包根聚合） |
| `core/waterprint/contracts/domain_exceptions.py` | L0 | 领域异常族公共核心单源（B3-c 批 2c 收敛件 2026-09-19：executor._DOMAIN_EXCEPTIONS[7 族]与 enumerate._ROW_DOMAIN_EXCEPTIONS[5 族]两份人工同步的交集四族[全 contracts 层定义]收敛——新增 contracts 层族改此单源自动同步两消费面，graph/registry 层语境族各消费面组合尾自管；latent 不对称[executor 不含 InvalidFormulaError]挂账 G1 见定案 docs/design/2026-09-19_b4-twins-convergence-design.md） | 同包 flow/quality/sludge/manifest 四族定义 | DOMAIN_EXCEPTIONS_CORE（经包根聚合） |
| `core/waterprint/registry/formulas/__init__.py` | L1 | 公式注册表子包聚合正门（B2-5 拆自 formulas.py 机制件，37 行——幂等哨兵批次行在此）：spec→store→apply 序聚合+九名 re-export（D1-B：条目留 manifest 原位，命名空间/直名两消费形态不断；私有名不进正门——store 持有） | 子包三机制件 | FormulaSpec、InvalidFormulaError、ValidationReport、register、by_id、validate_all、norm_ref_of、apply、apply_batch |
| `core/waterprint/registry/formulas/spec.py` | L1 | 公式规格件（B2-5 拆分，143 行）：FormulaSpec 五字段声明面+构造守卫（D6 归一+T3A-01 快照+ARCH1 D1d 形态守卫）+表达式 DSL 解析器（D7①②——split/受限 parse+子集收窄） | 单元包 manifest 构造字段值 | FormulaSpec、InvalidFormulaError（子包内共享载体） |
| `core/waterprint/registry/formulas/store.py` | L1 | 公式登记件（B2-5 拆分，145 行）：_REGISTRY 进程内唯一真源+登记期静态校验①~⑥（D7）+查询面（by_id 未知 id 拒；norm_ref_of=UF-43① 反查口；validate_all 收集不抛）+ValidationReport | 经 register 登记的 FormulaSpec | register、by_id、validate_all、norm_ref_of、ValidationReport |
| `core/waterprint/registry/formulas/apply.py` | L1 | 公式求值件（B2-5 拆分，188 行）：apply/apply_batch 唯一正门（R3/R6——禁绕过直抄公式代码；标量私核与批量 N=1 同实现 UF-36；形状合约 A2-03/G1-03；kernel 汇合点在此——N=1 标量快路径） | formula_id+bindings+ctx+TraceSink | apply、apply_batch |
| `core/waterprint/registry/formulas_kernel.py` | L1 | 公式数组求值核（批 13-A：ADR-011 D1/D3 落地——公式算术子集 ndarray 语义 walker+GR-37 errstate 三态 raise 承接+isfinite 二道网+域拒逐行重放[eval_checked 同缓存树 N=1 重放——探针位级恒等实证]；异常中性——消息构造与 NaN 政策归 formulas 正门；N=1 标量快路径在 formulas._apply_scalar 汇合[apply 与 apply_batch(N=1) 同一实现——UF-36 机器锚]） | formulas 缓存解析树+绑定数组（等长一维 float64） | BatchOutcome（values[域拒行 NaN]+issues）、KernelUnsupportedError、evaluate_batch |
| `core/waterprint/registry/dimensions.py` | L1 | 维度字段注册表（字段ID/单位/显示键/分类；dtype_of 已实现 T4 D5+dim 归一 D6，R1a 查询钩子经 bind_dimension_lookup 装配 bind-once；B3 R3：数据登记面 pool_length+八批次组迁 dimension_specs.py，本文件 FieldSpec(*tup) 构造循环+bind 统一置全部登记后——import 副作用终态恒等） | 字段声明+dimension_specs 数据件 | FieldSpec、register_dimension、dimension_of、dtype_of、InvalidDimensionError |
| `core/waterprint/registry/dimension_specs.py` | L1 | 维度登记数据件（B3 R3 拆自 dimensions.py 数据登记面）：pool_length+八批次组参数字段纯五元组序列（field_id/dim/unit/i18n_key/category 位置序=FieldSpec 构造参数序；dim 为 DimKey 成员名字符串——D6 归一语义保；批次组注释随迁）——零 import dimensions 环免疫 | 无（纯数据） | DIMENSION_SPECS 五元组序列（dimensions.py 构造循环消费；登记守卫=转写安全网） |
| `core/waterprint/registry/assumptions.py` | L1 | 设计假设清单唯一真源（B2-4 数值面 YAML 外置：守卫面+装载器+schema 校验，≤200 行；装载面=源树回溯优先+CWD 上溯兜底——受控三态仓内/CI/容器，残缺包 fail-fast；UF-08 loop.* 引擎参数三条 T7a 冻结语义随数据包迁移） | data/assumptions/ YAML 数据包+项目覆盖 | Assumption、AssumptionSet、DEFAULT_ASSUMPTIONS（22 条：21 YAML 装载序+design_map 伴生注入尾挂 1，[0] safety.superheight 锚不动）、assumption、TuningImpact、InvalidAssumptionError |
| `data/assumptions/manifest.yaml` | 数据包 | 假设数据包清单（B2-4：ordered_files=装载序载体——序使 safety.superheight 居 [0]，对 coefficients 文件名排序先例的批准偏离；schema_version 装载器只认现值） | — | ordered_files（4 件序）+schema_version |
| `data/assumptions/safety.yaml` | 数据包 | 安全域假设条目（1 键：safety.superheight——T5 D5 种子数值红线，与 factors.yaml factor.screen.superheight 双真源同步义务） | — | 1 条目（六键+tuning_impact 两键） |
...
| `core/waterprint/graph/executor.py` | L3 | 图执行编排（工况×拓扑×传播×回路；层-SCC 调度/DSL 工况映射/UF-42 投影/R5 异常隔离；T7b 实现；B3 R2：DSL 域/投影域拆 executor_dsl.py+executor_projection.py 本文件同名再导入[InvalidExecutionError 定义面在 dsl 修正①]，回路域/_RunState/execute_graph 留守；TD1 2026-09-09：输入装配域六件[_LOOP_KEYS/_NullSink/_endpoint/_edges_from_design/_loop_config/_unit_params]迁 executor_assembly.py[缝 A 预算减压]，本文件同名导入保引用连续） | design 图+单元注册表+工况集+RunEnv | execute_graph、UnitRegistry（协议）、InvalidExecutionError（dsl 再导出）、_apply_mappings/_dims_of/_snapshot 等 DSL·投影五符号（同名再导出）、PlantResult（design_hash 空串占位由 app 回填）；B12：单点路径缓存取或算+trace 重放+回路组旁路（use_cache=False——组内迭代中间值入缓存=假收敛），键材料 design_fingerprint/condition_key 经 _ConditionContext 下发 |
| `core/waterprint/graph/executor_dsl.py` | L3 | 执行器 DSL 域件（B3 R2 拆自 executor.py）：InvalidExecutionError 定义面（修正①——dsl→executor 反向环根除）+_POOL_KEY/_dotted/_rule_names/_apply_mappings（ADR-007 工况映射受限求值，docstring 随迁零行为变），executor.py 同名再导出=消费面零改动 | rule 串+params 全量+ConditionMapping+OperatingCondition | 求值后 params（bool→float 归一）+InvalidExecutionError（executor 再导出） |
| `core/waterprint/graph/executor_projection.py` | L3 | 执行器投影域件（B3 R2 拆自 executor.py）：_dims_of/_snapshot（UF-42 投影表——三键槽流量+指标键水质+dims GR-02 有限性校验），领域异常经 executor_dsl import（修正②同向无环），executor.py 同名再导出 | UnitResult+unit_id | UnitResultSnapshot（executor 再导出） |
| `core/waterprint/graph/executor_assembly.py` | L3 | 执行器输入装配域件（TD1 缝 A 拆自 executor.py，行为零变更纯搬迁——B3 R2 伴生件第三例）：_LOOP_KEYS/_NullSink/_endpoint/_edges_from_design/_loop_config/_unit_params（端点解析/边推导/回路配置/单元参数装配；跨件私有引用沿 executor_dsl 先例非包外公开契约，executor.py 同名导入保引用连续） | DesignState 图数据+RunEnv.engine_params+Unit manifest | PortRef/Edge 元组/LoopConfig/params 装配中间结构 |
| `core/waterprint/graph/incremental.py` | L3 | 脏传播与缓存（仅优化，字节级等价全量）——B9 首层实装：CacheKey 五字段 frozen 键材料+Scope 四栏（changed/downstream/loop_groups/full_graph——全图回落自包含）+recompute_scope 范围判定语义层（参数 diff+下游 BFS+回路不动点扩张；拓扑/其余键变更保守全图回落）；B12：executor 已接同指纹命中、recompute_scope 消费面挂账批 11、ResultCache 实现迁移 graph/cache.py（R1 字节级等价 oracle 留白——批 11 实装；B16 措辞勘误防与裁定2 被否决「不背书」注记案混淆） | 旧新 DesignState（nodes/edges diff） | Scope 重算范围+CacheKey（B12 经包根聚合） |
| `core/waterprint/graph/cache.py` | L3 | 结果缓存件（B12 实装；B16 N14 增容量整数校验+键材料纪律注记后 160 行）：进程内 LRU（容量 512 可配，条目不可变失效=键不再命中）+CachedUnitRun 值载体（UnitResult+trace 节点——命中重放保冷/暖含 trace 字节级等价）+CaptureSink 冷跑捕获腔+design_fingerprint（graph 内自算键分量——与 L4 ReproTriple.design_hash 不同源）；落盘 arrow 面挂账（512MB=落盘磁盘预算裁定）；每 worker 进程一份单线程消费前提 | CacheKey+UnitResult+TraceSink 协议 | ResultCache/CachedUnitRun/CaptureSink/design_fingerprint/default_cache（经包根聚合） |
...
| `core/waterprint/solution/design_map.py` | L3 | 可行域引导正门产物件（FD PD1~PD5：resolve_axes 轴声明归一[长度 1~2/field_id 属该单元/range 缺省=manifest ⊆ 闭区间校验/step 缺省=derive_step (max-min)/10]+解析期点数预算 ensure_budget[总点数超 max_points 2500 先于 build_grid 拦截]+双源可行掩码 feasible_mask[行非 NaN ∧ 约束通过——全绿≠无信息]+DesignMap 结构化产物[1D 段合并/2D 掩码声明序 row-major 转置归位/constraint_coverage full·degraded 降级不拒/diagnosis 逐轴块——无可行段恒 None 不编造]+确定性 JSON 双跑字节同；grid.build_grid keyword-only guard_base=False 跳过 R1 组合数护栏的唯一消费方） | 轴声明+网格求值行（enumerate 同源——R1 单实现双用）+双源掩码 | DesignMap、DesignMapOptions、DesignMapTooLarge、InvalidDesignMapError（app 再导出=server 单入口消费面）+derive_step/axis_point_budget/feasible_mask/widest_segment |
...
| `core/waterprint/drafting/catalog.py` | L3 | 图纸目录 sheet 纯函数（M6 案乙 B 形态：全厂总图文件内嵌目录页——CatalogRow 行元组→表题/表线/表头/数据文字实体；sheet_origin_below 自包围盒下方派生放置 m 域；SITE_SHEET_NO/DEFAULT_SCALE 常量桥供 L4 接线侧正门取值零第二真源；图号零持久化） | CatalogRow 行+origin_xy | DXF 实体组（EntityGroup） |
...
| `core/waterprint/geometry/scene.py` | L3 | 场景图 schema 与装配（<100ms；L5a site 级：site_design 摆放/未摆放不进图/红线 polyline 图元/水面+渠道接线收口；L6 roads/corridors 条带收编：kind=strip 分段四边形角点 core 预计算压平 dims，semantic=site_road/site_corridor:{kind}，全退化=零节点；C2-3d：edges 关键字管廊装配接线[pipes.py 段抽离]+SCENE_VERSION 步进 -6） | 结果 schema+假设+SiteDesign（None/空=回退 X 轴排布） | SceneGraph JSON |
| `core/waterprint/geometry/pipes.py` | L3 | 管廊装配（C2-3d：design.edges→单元间高架方管——两色制 pipe_water/pipe_sludge/架顶=最高构筑物顶+1.2 净空/管端缩进至结构缘/畸形-自环-inlet-未摆放端防御跳过/单元对去重） | edges 原始字典+单元中心/顶标高/占位长 | tuple[Node, …] |
...
| `core/waterprint/geometry/spacing.py` | L3 | 间距校核裁判（L4b AABB→SPC2 OBB 精确净距纯函数——点-边枚举 32 对距 min+归零判定先行[边对相交/一方全含→0.0]；旋转 0° 恒等旧 AABB 式回归锚；footprint None=不入对+uncalculated 降级；阈值结构化透传零 DSL——kb expression 解析归 server 装配；webapp siteGeometry measureToNearest 同式镜像） | placements（unit_id→x/y/rotation）+footprints（unit_id→(w,h)|None）+thresholds（SpacingThreshold 三元组） | SpacingReport（violations 字典序全序+uncalculated sorted） |
| `core/waterprint/geometry/boundary.py` | L3 | 用地红线越界校核裁判（SPC2 D3 独立模块纯函数：OBB 四角全在多边形内含边上[容差 1e-9 幂积形态]=合规、任一角严格在外=违规；射线法奇偶+两段式贴边归内+顶点序无关+凹多边形支持；boundary 空/len<3=零违规零报错；message「unit X 有 N 个角点超出红线」三字段不发明越界深度；severity 透传零 DSL——kb boundary_check 解析归 server 装配） | placements+footprints（同 spacing 口径）+boundary（顶点序闭合多边形，消费方补闭合段）+rule（BoundaryRule severity） | BoundaryViolation 序列（unit_id sorted 全序——确定性） |
...
| `core/waterprint/app.py` | L4 | 用例编排：装配 + run_full_calc/load_project+save_project + run_enumeration（唯一装配点，UF-33 方案 A 已落 M2-SOL；T7a 份额=load/save 薄封装+RunEnv 再导出；T7b 份额=assemble/run_full_calc+AssembledGraph/ResultBundle 两字段子集+design_hash 回填+_engine_params 投影+M-3 migrate 路由已落；M1a 份额=_unit_params 系数投影+_CoefficientsUnit 包装（factor.*/removal.* 并入单元 params，D4 裁决）；M2-SOL 份额=装配 grid 档命中校验[Ruling ④]+run_enumeration 编排+app_enumeration 伴生件再导出） | 项目+工况+环境 | AssembledGraph、ResultBundle、assemble、run_full_calc、run_enumeration、export_artifact、EnumerationOptions/EnumerationOutcome（再导出）、ArtifactKindNotReady、InvalidAssemblyError、load_project（migrate 路由+SERVER D2 双闸收口=委托 io 正门）、save_project、RunEnv（再导出）、Constraint/InvalidProjectError/DEFAULT_ASSUMPTIONS/discover_units（SERVER 批+META1 再导出——server 消费面单入口，__all__ 21 名〔B3 陈数勘误：原记 19 名系陈数（META1 后再扩 InvalidSitePlanError/Node 两名），python -c len(waterprint.app.__all__)〕；B3 R1：装配域十符号迁 app_assembly.py，消费面符号四名（AssembledGraph/assemble/InvalidAssemblyError/_unit_params）经 app 再导出=消费面零改动；域内私有六符号（_endpoint/_edges/_checked_units_eligibility/_FACTOR_SHARED_PREFIX/_CoefficientsUnit/_check_grid_hits）零外部消费（B3 总控 grep 实证——不设再导出；「全部被迁符号同名再导出」规格句勘误为按实际消费收敛〔B3 R 轮 G1-02/G2-01〕） |
| `core/waterprint/app_enumeration.py` | L4 | UF-33 用例面伴生件（M2-SOL D2；app.py 500 行预算宪法 §2 拆分正解）：枚举选项/产出类型 + export_artifact/ArtifactKindNotReady 再导出（PROFILE3 export 族拆出 app_export.py——分发/路由/装配职责随迁）+ upstream_context 上游快照重建（execute_graph 既有产物 UF-42 反解；零 app 依赖防 import 环，类型面注解消费 solution 三模块+trace 正门——I-4 R1 修正；层序登记=SERVER D1 已落[同层并列+ignore_imports 唯一伴生边豁免]） | app 装配产物+结果 | EnumerationOptions、EnumerationOutcome、UpstreamSource、export_artifact、upstream_context、ArtifactKindNotReady、Constraint（SERVER D1 再导出） |
| `core/waterprint/app_export.py` | L4.app 伴生件（PROFILE3 2026-09-08 自 app_enumeration 拆分——export 族[export_artifact 分发/_export_dxf 三形态路由/_export_profile_dxf 纵断装配/h·v 比例终闸];行为零变更纯搬迁;app_enumeration 再导出消费面零改动） | PlantResult+路由 options | export_artifact/ArtifactKindNotReady |
| `core/waterprint/app_assembly.py` | L4 | 装配域伴生件（B3 R1 拆自 app.py 500 行预算——app_enumeration 先例第二例）：InvalidAssemblyError/_endpoint/_edges/AssembledGraph/_checked_units_eligibility/_FACTOR_SHARED_PREFIX/_unit_params/_CoefficientsUnit/assemble/_check_grid_hits 整域逐字搬运（D4 系数投影+受检资格+Ruling ④ grid 档命中随迁；assemble 的 discover_units 消费随域=units_lib 边域内必需），app.py 顶部显式清单再导入（禁 import *；零 app 反向依赖防环） | ProjectFile+RunEnv+单元发现结果 | AssembledGraph、assemble、InvalidAssemblyError、_unit_params 系数投影（app.py 再导出——消费面零改动） |
| `core/waterprint/app_trust.py` | L4 | 可信度诊断装配域伴生件（P2 次批 ADR-012——app_assembly 拆分先例同构）：DiagCollector（DiagSink 采集）+TrustContext（装配上下文打包）+_mass_balance_of（拓扑重建纯投影 R1~R4 流体分线/工况取值口径）+_effluent_of（summary×标准×margin）+build_diagnostics | PlantResult+AssembledGraph+ConditionSet+标准族+RunEnv | DiagnosticsReport（app.run_full_calc 组装——standards 可选注入 D4） |
| `core/waterprint/app_energy.py` | L4 | 能耗药耗全厂聚合投影伴生件（B4-2a 碳核算前置一 2026-09-19，ADR-024；app_trust 拆分先例同构第五例——不进 import-linter layers 契约[unconstrained 同款]）：energy_summary_of 纯投影=逐工况 Σ 单元 dims 键名约定聚合（e_aeration/e_pump/e_stir→power_*_kwh_d 分项+total；m_pac/m_pam/w_pam/m_seed_net→dose_*_kg_d——w_pam 并入 PAM 族、m_seed 毛耗不聚合[磁种循环投加非净消费]）；sparse 有则录无则略（_summary_of 同口径）；app.py _with_energy 合并注入 summary 开放映射槽位[result_schema 零改——ADR-012 D1 总线稳定红线保持] | PlantResult.conditions（单元快照 dims） | energy_summary_of（app.run_full_calc 消费——summary 能耗药耗平键族） |
| `core/waterprint/app_opex.py` | L4 | 运行成本全厂投影伴生件（B4-2b 碳核算前置二 2026-09-19；app_energy 先例同构第六例——不进 import-linter layers 契约[unconstrained 同款]）：opex_summary_of 纯投影=逐工况 F1~F3 年运行成本键（F1 cost_electricity_yuan_a=power_total_kwh_d×factor.opex.electricity×factor.opex.days_per_year；F2 cost_chemicals_yuan_a=Σ在场 dose_{pac,pam,seed}_kg_d×factor.opex.{pac,pam,magnetic_seed}[单价元/kg 口径÷1000 消除]；F3 cost_opex_yuan_a=Σ在场分项[无分项不立合计]）+ _with_opex 合并注入（_with_energy 同语义——同工况字典 update、base 键族优先、命名域无交集）；sparse=单价键 factor.opex.* 前缀列举判在场[单键缺席跳该分项/全缺席无 cost 键]；可算面=电费+药剂费（年化 365 天连续运行——factor.opex.days_per_year 承载，与 B4-2a ×24h/d 同源），人工/维修/污泥处置/折旧/吨水键显式挂账不造零值键；summary 开放映射槽位[result_schema 零改——ADR-012 D1 总线稳定保持] | summary 能耗药耗合并面（power_*/dose_* 平键）+RunEnv.coefficients（CoefficientsView——factor.opex.* 五键） | opex_summary_of、_with_opex（app.run_full_calc 消费——summary 年成本平键族） |
...
| `server/waterprint_server/settings.py` | 环境配置（路径基点 safe_child 分量白名单/上限 fail-fast/优先级值域/ENGINE_VERSION；R2A 批1：api_token 字段[env WATERPRINT_API_TOKEN 空=鉴权关]+长度下限 validator[API_TOKEN_MIN_LENGTH 真源 62¹⁶≈4.8×10²⁸]+回环绑定防线[空 token 仅许 {127.0.0.1,::1,localhost}]，262 行） | env | Settings、get_settings、safe_child、ensure_directories、ENGINE_VERSION |
| `server/waterprint_server/auth.py` | token 鉴权依赖（R2A 批1：verify_token 受保 19 端点 Bearer 通道+verify_token_sse events 双通道[header 或 ？token=]+AuthError→401 统一错误体；compare_digest 常量时间比对；OpenAPI httpBearer/sseTokenQuery 双安全 scheme 真源；B6 批[2026-09-06]：OR 无优先级明文化+命中通道 debug 日志[不记 token 值]+规格修订[限流仅限 SSE 面——Ruling 2026-09-06 ② 授权链记档；匿名配额 DoS 残余风险知情接受]，124 行） | Request（ctx.settings.api_token） | 路由依赖（main include 挂载）+AuthError |
| `server/waterprint_server/sse_limits.py` | SSE 四维限流器（B6 批 2026-09-06 简报 D1~D7：RateLimitedError→429 统一错误体+Retry-After 建议头[main 入表]；全局并发/每任务/每项目订阅计数+建连令牌桶[突发 2×rate 固定]；check+add 同步临界区零 await；建连闸 check_connect 经 include 级依赖序先于 verify_token_sse[401 风暴前置压制]；鉴权关四维身份无关全额生效；旋钮 int|None=关[settings 五旋钮]；零 fastapi 依赖[services 层可安全 import]，167 行） | Settings 四旋钮（from_settings）+events 建连/订阅/断开时点 | RateLimitedError+SseLimiter（ServiceContext.sse_limiter 挂载） |
| `server/waterprint_server/errors.py` | 统一错误体类型化声明（PL-03 契约枚举 GOV5 2026-09-12：ErrorResponse {detail, error_type}——openapi responses 声明面[lifecycle 三端点 404/409+trust 404]；运行时行为零变化，响应体构造仍在 main._make_handler 唯一翻译处，26 行） | 无（纯 pydantic 模型件） | ErrorResponse（routers responses 声明引用） |
...
| `server/waterprint_server/routers/projects.py` | 项目 CRUD+生命周期八端点（薄协议转换，254 行——P0-1：CreateProjectRequest +name/ProjectSummaryResponse +name；P0-3：validate 端点可选 body=待存草稿校验；P2 生命周期批 2026-09-12：copy/rename/DELETE 三端点+RenameProjectRequest/DeleteOutcomeResponse） | pydantic 请求 | 响应=服务 dataclass |
| `server/waterprint_server/routers/calc.py` | 计算/枚举八端点（幂等/取消/分页+FD design-map 同步直返[asyncio.to_thread 承载防事件循环阻塞——计算面首例；axes 长度 1~2=pydantic 422 面]+P2 次批 GET trust 可信度报告[ADR-012 D8 2026-09-12]+P2 第三批 GET compare 多工况对比矩阵[ADR-018 D5 2026-09-12——指标×工况聚合端点集 32→33]） | 任务请求+轴声明 | 任务句柄/TaskStatus/SolutionPage/DesignMapResponse |
| `server/waterprint_server/routers/exports.py` | 导出五端点（stale 守门/文件流/批量转任务句柄，151 行） | 导出选项 | 文件流/ExportHandle |
| `server/waterprint_server/routers/events.py` | SSE 两端点（X-Accel 头/事件 JSON 化/断连清理+AUDIT2 FIX1 I-2 开流前置探测[分量 422/未知 id 404]，139 行） | 任务订阅 | text/event-stream |
| `server/waterprint_server/routers/scene.py` | 场景图端点（FE1：GET /api/scene/{project_id} 一端点[condition_key 可选——缺省=排序首键回显]；response_model=SceneResponse[SceneGraph 四字段+stale——AUDIT2 FIX1 C-1]，51 行） | project_id+condition_key | SceneResponse |
| `server/waterprint_server/routers/units.py` | 单元元数据端点（META1：GET /api/units+GET /api/assumptions 两端点——静态只读 lru_cache 直投；response_model=服务层冻结模型再导出，63 行） | 无（静态目录） | UnitCatalog/AssumptionCatalog |
...
| `server/waterprint_server/routers/debug.py` | debug 观测端点（B4-1 实现批 2026-09-19：《裁决书》方案五①——GET /api/debug/ops-chain/{project_id} 操作链观测面单端点[35→36 破面裁决书授权]；零业务薄封装[trust/compare 同制]，聚合行为全在 services.ops_debug；只读零任务副作用；PL-03 契约枚举 404 responses 声明，61 行） | 项目 id（路径） | OpsChainResponse |
...
| `server/waterprint_server/services/_shared/__init__.py` | services 同层共享子包包根（B3-a 同层晋升正门首件家——《裁决书》方案二批 2a：共享件被同层消费、自身禁 import 任何消费方[星型单向]；薄壳零聚合——单件子包消费面直连模块路径，件多再议 __init__ 聚合[core registry/formulas 聚合正门先例]，14 行） | 无 | 无 |
| `server/waterprint_server/services/_shared/latest_calc.py` | 最近完成计算结果集共享取数件（B3-a：scene/elevation/cost/exports_io/compare/trust 六份 def 复制+exports 转手 import 七消费面收敛单源——准入五条全过[≥2 真实消费/无业务分支/命名中性/旧复制同批删除/file-contracts 登记]；R1 取数=注册序最末 done calc 实时取[UF-37]；ENG4 D2 二元组口径[携 task_id 信息超集——溯源回显面 result bag 无该键，单值面解包丢弃分量]；R2 无结果 raise not_found 注入[消费方领域 404 异常类——异常→HTTP 码经 main domain_error_codes 类名义表，类型与消息文本收敛前后逐字恒等]；R3 星型单向唯一依赖=services/__init__ ServiceContext[包根不回指子模块无环]，67 行） | ServiceContext+project_id+not_found 异常工厂 | (task_id, result) 二元组 |
| `server/waterprint_server/services/exports.py` | 导出用例编排主件（stale 409+force 标注/确定性命名[FE9 R1：dxf 单产物附 unit 分量防同名覆盖]/单产物上限 1 转任务/元数据边车；FE9 D2/D3/D4：模板闸收窄至 calcbook+单产物 options 透传 unit_id/condition_key[R3 严格化]+kind 后缀映射 dxf→.dxf；ENG7 P3a/P3b：五纯函数+ExportMeta+三常量+命名闸异常迁 exports_support[顶部 import+`__all__` 透传保公开面]+create_export 抽批量对偶拒绝闸与图纸族 kwargs 组装两子函数消 PLR0915 豁免；SVRB：items 逐项 unit_id 归一[item 覆盖批级]+payload 增 project_path[提交时绝对路径]/design_digest[快照信任留痕]+_build_drawing_kwargs 迁 jobs/export_kwargs[worker 共享真源]+拒绝闸改载 ifc 批内 unit 一致小闸[原两族删除]；B7 笔①：异常族四类+ExportHandle 迁 exports_support+读面两函数[resolve_export_file/list_exports]迁新件 exports_registry[顶部 import 透传再导出保公开面——`__all__` 10 名恒等，main/routers/测试零改动]；PROFILE2：sheet 选项提取透传[core 纵断路由；R 轮 P2A2-1：item 级并入 item 覆盖批级]+命名 sheet 分量[防总图纵断同名覆盖]+批量面 sheet 诚实拒[worker 透传挂账]；PROFILE3：批量面 sheet 解锁+sheet/h/v 逐项归一+命名逐项化+h/v 透传；R 轮互斥继承压制+闸更名；TD1 PD4-bis：五私有 helper+_TEMPLATE_KINDS 迁 exports_io.py[IO 支撑域——support R-1 纯度不可承载；B7 D1 内聚裁定随预算减压改裁]；B3-a：_latest_calc_result 改经 services/_shared/latest_calc 共享件[not_found 注入 ExportSourceNotFoundError——行为零变]，386 行） | 导出请求 | ExportHandle/ExportMeta |
| `server/waterprint_server/services/exports_support.py` | exports 纯函数与常量支撑（ENG7 P3a：命名分量白名单/确定性命名/unit 严格化/边车文本/批量 IPC items 五纯函数+ExportMeta+kind 三常量+InvalidExportRequestError[命名闸 raise 面依赖随迁防循环 import]——零 IO 零全局态，exports 透传再导出；SVRB：批量 items unit_id 逐项真源[item 覆盖批级——归一位在 create_export]+ifc 项边车 {ifc} 补齐[dxf={dxf,dwg} 并存]；B7 笔①：异常族四类[StaleExportError/ExportSourceNotFoundError/ExportTemplateMissingError/ExportFileNotFoundError]+ExportHandle 自 exports.py 逐字迁入[经 exports 透传再导出+exports_registry raise 消费]；PROFILE2：_sheet_of 提取+命名 sheet 分量；PROFILE3：_scale_text_of+reject_bad_scale_forms 整批原子 422+命名 h/v 段；R 轮 reject_bad_route_options 扩（非字符串/互斥收单闸）；R 轮 try/except 闭合+互斥闸（reject_bad_route_options），317 行）| 文件名分量/导出请求面（str·Mapping） | 确定性文件名/边车文本/批量载荷 |
| `server/waterprint_server/services/exports_io.py` | exports IO 支撑件（TD1 PD4-bis 拆自 services/exports.py，行为零变更纯搬迁）：_template_for/_reject_conflicting_batch_pairs/_write_meta/_post_export_dwg+_TEMPLATE_KINDS（模板解析/ifc 批内 unit 一致闸/边车原子写/DWG 转换挂点——消费 ctx 面，与 support 纯函数岛分工；B3-a：_latest_calc_result 迁出收敛 services/_shared/latest_calc[六处复制单源化]——ExportSourceNotFoundError import 随迁除名，106 行） | ServiceContext+导出请求面 | 模板路径/边车落盘/DWG 挂点 |
| `server/waterprint_server/services/exports_registry.py` | exports 产物注册表读面（B7 笔①预拆新件：resolve_export_file 下载校验正门[恒等闸→后缀闸→stem 闸→存在性双闸→绝对路径]+list_exports 清单扫描[元数据边车]自 exports.py 逐字迁入——经 exports 顶部 import 透传再导出，消费面零改动；依赖 exports_support 下载两闸常量/ExportMeta/两 raise 面异常+ServiceContext，100 行） | ServiceContext+下载文件名/项目 id | 校验后产物绝对路径/ExportMeta 元组 |
| `server/waterprint_server/services/scene.py` | 场景图用例（FE1：最近结果集取数[B3-a 起经 services/_shared/latest_calc 共享件——原 exports 同款模式复制收敛]/假设合成视图[worker._build_env 同款]/core.build_scene 纯投影+AUDIT2 FIX1 C-1 stale 旗标[SceneResponse=四字段+stale]+C2-3d edges 管廊透传；无结果 404/工况非法 422/确定性继承，141 行） | 项目 id+工况键 | SceneResponse |
| `server/waterprint_server/services/constraints.py` | 约束知识库装载投影（CP1 D4：constraint_kb 1.0.0 起草态 18 条[过滤 6+出水参考 12]——八键逐字投影零数值字面量/fail-visible 四路[缺失·损坏·缺键·重复→RuntimeError 禁静默空表]/路径键 lru_cache 单例+双跑字节同；DSL 校验边界归 core apply 运行期守卫） | data_dir | ConstraintCatalog |
...
| `server/waterprint_server/services/trust.py` | 可信度报告用例（P2 次批 ADR-012：latest done calc 取数[B3-a 起经 services/_shared/latest_calc 共享件——携 task_id 二元组，原 scene 同款模式复制收敛]+plant 反序列化+diag 件读取[缺失/损坏→diagnostics_available=False 降级呈现 R2——禁伪造空诊断]+warnings 全工况×全单元六键聚合[WarningEntry 复用+unit_id 定位]+warning_counts 按级计数+stale[result_is_stale 四端点同口径]；无结果 404 附重算指引；301 行） | 项目 id | TrustReportResponse |
| `server/waterprint_server/services/compare.py` | 多工况对比矩阵用例（P2 第三批 ADR-018 D5：latest done calc 取数[B3-a 起经 services/_shared/latest_calc 共享件——携 task_id 二元组，原 trust 同款模式复制收敛]+plant 反序列化+指标行投影[out_dims 声明面×工况取值——NaN 无值键不出+全空行不呈现]+警告计数行[全工况×全单元总数稀疏面]+stale[result_is_stale 同口径]+design_hash 回显[结果件 repro 真源——FE 锁定基准 pinned_hash 比对面 D3]；无结果 404 附重算指引） | 项目 id | CompareReportResponse |
| `server/waterprint_server/services/ops_debug.py` | 操作链 debug 用例（B4-1：任务时间线[manager 注册序+task_status 状态正门复用——error_code 回填/实时 stale 比对口径单源]+最新 done calc 三源深度聚合[latest_calc_result 共享件第七消费面填槽 B3-a；diag 降级同 trust ADR-012 R2；trace 聚合统计 by_condition/by_unit/by_formula 字典序确定性]；无 done calc=latest_calc 空块 200[诊断面语义与 trust 404 消费面分立]；哨兵异常类注入 not_found 槽禁逃逸，292 行） | ServiceContext+project_id | OpsChainResponse |
...
| `server/waterprint_server/services/elevation.py` | 高程纵断用例（FE7：最近结果集取数[B3-a 起经 services/_shared/latest_calc 共享件——原 scene 同款模式复制收敛]/假设合成视图/head_losses 空段+±0.00 相对标高+build_profile+evaluate_pumping 装配/crest_elev 服务端投影；无结果 404/工况非法 422/确定性继承+AUDIT2 FIX1 C-1 stale 旗标，274 行） | 项目 id+工况键 | ElevationResponse |
| `server/waterprint_server/services/cost.py` | 概算用例（FE8：最近结果集取数[B3-a 起经 services/_shared/latest_calc 共享件——原 scene 同款模式复制收敛]/load_prices→load_fee_rules→takeoff→build_estimate→check_indicators 四模块装配/design_scale 经 pint 换算服务面注入/name_zh 单价包直投；无结果 404/工况非法 422/确定性继承+AUDIT2 FIX1 C-1 stale 旗标，372 行） | 项目 id+工况键 | CostResponse |
...
| `server/waterprint_server/jobs/manager.py` | 任务注册表与调度（状态机单向/优先级堆同级 FIFO/幂等键/mp.Queue→asyncio 桥[run_coroutine_threadsafe]/文件取消令牌/SSE 背压丢旧保新；ENG5 四时机落盘[submit 初档/running 迁移/终态/cancel·shutdown 的 queued 终态]+records 数据类再导出面[__all__]；SVRB：终态 result=已产 outcome 灌入[done/cancelled 两态——§2.3 缺陷收口，cancelled 不再丢弃 files/failures 清单]；B3 R4：_TaskRecord 迁 records.py 本模块 import 再导出[桥三方法持 manager 状态留守]；B4-1：finished_at 公开投影方法[诊断时间线面——WP4 内存值，重启恢复=恢复时刻新租约语义注记]，488 行） | TaskRequest | TaskStatus/Event 流 |
| `server/waterprint_server/jobs/registry.py` | 注册表落盘序列化面（S2 R1 拆分+ENG5：TERMINAL_STATES 终态单源[manager._TERMINAL 同源]/PERSISTABLE_STATES 可落盘全集/task_document 平字段文档[TaskStatus 同构+snapshot_hash]/write_record GR-38 原子写/mark_interrupted 非终态→failed[InterruptedByRestart] 变换/iter_restorable 恢复流[非终态变换产出+损坏·缺键·越界跳过 warning fail-visible]，267 行〔B3 陈数勘误：原记 187 系 WP4 扩面未回更〕） | registry_dir+平字段束 | JSON 落盘/记录流 |
| `server/waterprint_server/jobs/records.py` | 任务域公开数据类（ENG5 D4 拆分：manager 500 行预算 S2 R1 先例——UnknownTaskError/TaskRequest[kind 白名单 _KINDS+payload 快照守卫]/TaskHandle/TaskStatus[error_code 回填位]/Event；纯数据零逻辑，manager 经 __all__ 再导出面稳定；B3 R4：_TaskRecord 注册表条目自 manager 迁入[terminal/status 面；_TERMINAL 经 registry.TERMINAL_STATES 同源——records→registry 单向无环]，manager 顶部 import 再导出保模块属性面，145 行） | 无 | 数据类定义 |
| `server/waterprint_server/jobs/worker.py` | 进程池入口（run_task 三参 pickle 边界唯一面/kind 映射表集中一处经 app[UF-33]/RunEnv 协议适配器[UF-46]/阶段取消轮询[UF-49]/feather+serialize 原子落盘/导入零副作用；R2-C：export_batch dxf 项双产物挂钩〔sidecars 二道闸 K-03+转换前取消 K-02+D-02 后缀闸入 _safe_out_name〕；SVRB：批首 _load_project[project_path 通道——calc 同款]+逐项 _build_drawing_kwargs[与单产物等价]+ifc 放行三件[_EXPORT_KINDS/ifc 后缀特判/ifc 边车]+部分失败协议[逐项 try/except 收集 failures——部分失败=done+failures/全失败=raise 聚合]+stage 带 unit 段；B3 R5：数据包适配域三符号[DataPackError/_CoefficientEntry/_YamlCoefficients]迁 jobs/datapack.py 本模块 import 同名再导出=消费面零改动；B2②：enumerate grid_fields 载荷扩 [{key,dim,label_zh}]——dim/label_zh 自单元 manifest.params 按 field_id 查[label_zh=None 直传，降级兜底归显示层]；V2 GOV5 批尾：enumerate result +dim_fields 载荷[同形状——计算派生输出量列族自 manifest.out_dims 查，未声明键 label_zh=None 直传]；P0-2：enumerate result +unit_id[深链回填源]/+design_hash[漂移闸③源——calc 同键先例]；P2 次批[2026-09-12]：calc 装 standards[load_effluent_standards 数据装配 ADR-012 D4/D6]+diag 独立落盘 calc-diag-{task_id}.json+result +diag_file 键[装载经 app 再导出单入口——UF-33 契约 CI 实证]；ADR-018 D5[2026-09-12]：enumerate 载荷组装段[diagnosis 序列化+grid/dim 列族+result dict]整迁 jobs/enum_payload.py[500 行预算减压拆件——datapack/dwg 先例同制]，） ；PROFILE3：sheet/h/v item 级透传+stage sheet 段+条件 kwargs| payload+令牌 | 结果+进度 |
| `server/waterprint_server/jobs/export_kwargs.py` | export_batch kwargs 组装件（SVRB D2④：_build_drawing_kwargs 自 services/exports 迁入——worker 批量面与单产物路径共享真源[worker→services 上行违分层，services→jobs 向下合法]；ifc=assumptions+site_design/dxf=site_design/余空 dict；纯函数零 IO+导入零副作用[Windows spawn 铁律]，49 行） | kind+ProjectFile（worker 侧 load_project 重建） | core.export_artifact extra kwargs |
...
| `server/waterprint_server/jobs/datapack.py` | 数据包适配域件（B3 R5 拆自 worker 500 行预算：DataPackError/_CoefficientEntry/_YamlCoefficients 三符号整域逐字搬运——yaml/isfinite/Path/Mapping 四依赖零 worker 依赖自足[纯移动零行为变化]；CoefficientsView 协议镜像装载[B4 双胞胎]装载语义随迁；worker 顶部 import 同名再导出=消费面零改动，109 行） | 数据包目录（manifest.yaml 版本头+条目 *.yaml） | 系数适配器[worker._build_env 消费]+DataPackError |
...
| `scripts/family_parity_lib.py` | 同族一致性门禁共享库：声明面 AST 解析层（公式全表+平账计数/out_dims 表+条目计数，same_layer_lib 共享库先例拆件——解析与「跳过形状」计数同库） |
...
| `scripts/gen_status.py` | docs/status.md 生成器：全项目计数指标单一生成源（文件派生零环境依赖——门禁数/锁面键数/OpenAPI/ADR/UF 登记/数据包 data_version/快照锚点；确定性输出禁手编；`--check`=重生成字节比对退 1，CI status 零漂移步骤消费；生成器纪律同 ADR-016 族，不入 GATES） |
