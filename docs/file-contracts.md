...
| `server/waterprint_server/services/exports_io.py` | exports IO 支撑件（TD1 PD4-bis 拆自 services/exports.py，行为零变更纯搬迁）：_template_for/_latest_calc_result/_reject_conflicting_batch_pairs/_write_meta/_post_export_dwg+_TEMPLATE_KINDS（模板解析/最近结果集取数/ifc 批内 unit 一致闸/边车原子写/DWG 转换挂点——消费 ctx 面，与 support 纯函数岛分工，123 行） | ServiceContext+导出请求面 | 模板路径/最近结果集/边车落盘/DWG 挂点 |
...
| `server/waterprint_server/services/scene.py` | 场景图用例（FE1：最近结果集取数[exports 同款模式复制]/假设合成视图[worker._build_env 同款]/core.build_scene 纯投影+AUDIT2 FIX1 C-1 stale 旗标[SceneResponse=四字段+stale]+C2-3d edges 管廊透传；无结果 404/工况非法 422/确定性继承，153 行） | 项目 id+工况键 | SceneResponse |
