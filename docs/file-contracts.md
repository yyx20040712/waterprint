...
| `server/waterprint_server/routers/projects.py` | 项目 CRUD+生命周期八端点（薄协议转换，235 行——P0-1：CreateProjectRequest +name/ProjectSummaryResponse +name；P0-3：validate 端点可选 body=待存草稿校验；P2 生命周期批 2026-09-12：copy/rename/DELETE 三端点+RenameProjectRequest/DeleteOutcomeResponse） | pydantic 请求 | 响应=服务 dataclass |
| `server/waterprint_server/routers/calc.py` | 计算/枚举八端点（幂等/取消/分页+FD design-map 同步直返[asyncio.to_thread 承载防事件循环阻塞——计算面首例；axes 长度 1~2=pydantic 422 面]+P2 次批 GET trust 可信度报告[ADR-012 D8 2026-09-12]，199 行） | 任务请求+轴声明 | 任务句柄/TaskStatus/SolutionPage/DesignMapResponse |
