...
| 路径 | 唯一职责 | 输入 | 输出 |
|------|----------|------|------|
| `server/waterprint_server/main.py` | 应用工厂与生命周期（SERVER 已实装：进程池[initializer 进度队列注入]/统一异常映射 22 类基+DOMAIN_ERROR_CODES 名义表/契约自检端点==19/structlog/CORS/请求 ID） | Settings | ASGI app（create_app、app、DOMAIN_ERROR_CODES） |
