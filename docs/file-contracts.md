...
| `server/waterprint_server/services/units.py` | 单元目录+假设清单用例（META1：discover_units 32 包+D7 builtin 四 kind 投影 36 条+D1 中文名映射 36 条+DEFAULT_ASSUMPTIONS 21 条六字段取五；B2：ParamEntry 六字段投影——参数级中文真源=manifest label_zh 随六字段透传[builtin 无声明面=null 诚实缺省]；lru_cache(maxsize=1) 静态缓存，309 行） | core.app+contracts | UnitCatalog/AssumptionCatalog |
