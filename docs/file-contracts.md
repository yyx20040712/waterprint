...
| `server/waterprint_server/routers/projects.py` | 项目 CRUD 五端点（薄协议转换，167 行） | pydantic 请求 | 响应=服务 dataclass |
