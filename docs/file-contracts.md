...
| `core/waterprint/graph/incremental.py` | L3 | 脏传播与缓存（仅优化，字节级等价全量）——B9 首层实装（315 行）：CacheKey 五字段 frozen 键材料+Scope 四栏（changed/downstream/loop_groups/full_graph——全图回落自包含）+recompute_scope 范围判定语义层（参数 diff+下游 BFS+回路不动点扩张；拓扑/其余键变更保守全图回落）；ResultCache/执行集成挂性质接线批（R1 字节级等价本批不背书） | 旧新 DesignState（nodes/edges diff） | Scope 重算范围+CacheKey；ResultCache 挂账 |
