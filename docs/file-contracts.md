...
| `core/waterprint/registry/assumptions.py` | L1 | 设计假设清单唯一真源（B2-4 数值面 YAML 外置：守卫面+装载器+schema 校验，≤200 行；UF-08 loop.* 引擎参数三条 T7a 冻结语义随数据包迁移） | data/assumptions/ YAML 数据包+项目覆盖 | Assumption、AssumptionSet、DEFAULT_ASSUMPTIONS（22 条：21 YAML 装载序+design_map 伴生注入尾挂 1，[0] safety.superheight 锚不动）、assumption、TuningImpact、InvalidAssumptionError |
