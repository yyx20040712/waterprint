...
| `server/waterprint_server/services/_shared/latest_calc.py` | 最近完成计算结果集共享取数件（B3-a：scene/elevation/cost/exports_io/compare/trust 六份 def 复制+exports 转手 import 七消费面收敛单源——准入五条全过[≥2 真实消费/无业务分支/命名中性/旧复制同批删除/file-contracts 登记]；R1 取数=注册序最末 done calc 实时取[UF-37]；ENG4 D2 二元组口径[携 task_id 信息超集——溯源回显面 result bag 无该键，单值面解包丢弃分量]；R2 无结果 raise not_found 注入[消费方领域 404 异常类——异常→HTTP 码经 main domain_error_codes 类名义表，类型与消息文本收敛前后逐字恒等]；R3 星型单向唯一依赖=services/__init__ ServiceContext[包根不回指子模块无环]，62 行） | ServiceContext+project_id+not_found 异常工厂 | (task_id, result) 二元组 |
