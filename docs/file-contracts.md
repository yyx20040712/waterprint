...
| `server/waterprint_server/routers/exports.py` | 导出五端点（stale 守门/文件流/批量转任务句柄，148 行） | 导出选项 | 文件流/ExportHandle |
...
| `server/waterprint_server/services/exports_registry.py` | exports 产物注册表读面（B7 笔①预拆新件：resolve_export_file 下载校验正门[恒等闸→后缀闸→stem 闸→存在性双闸→绝对路径]+list_exports 清单扫描[元数据边车]自 exports.py 逐字迁入——经 exports 顶部 import 透传再导出，消费面零改动；依赖 exports_support 下载两闸常量/ExportMeta/两 raise 面异常+ServiceContext，99 行） | ServiceContext+下载文件名/项目 id | 校验后产物绝对路径/ExportMeta 元组 |
