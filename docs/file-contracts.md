...
| `core/waterprint/registry/assumptions.py` | L1 | 设计假设清单唯一真源（默认值+出处；UF-08 loop.* 引擎参数三条 T7a 冻结） | 假设声明+项目覆盖 | Assumption、AssumptionSet、DEFAULT_ASSUMPTIONS（22 条：[0] safety.superheight 锚不动+loop.* 三+kb/工艺面十八——FD 批 21→22 增 solution.design_map.max_points[伴生件注入]，A2-N-03 勘正）、assumption、TuningImpact、InvalidAssumptionError |
