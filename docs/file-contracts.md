...
| `server/waterprint_server/services/exports.py` | 导出用例（stale 409+force 标注/确定性命名[FE9 R1：dxf 单产物附 unit 分量防同名覆盖]/单产物上限 1 转任务/元数据边车；FE9 D2/D3/D4：模板闸收窄至 calcbook+单产物 options 透传 unit_id/condition_key[R3 严格化]+kind 后缀映射 dxf→.dxf；ENG7 P3a/P3b：五纯函数+ExportMeta+三常量+命名闸异常迁 exports_support[顶部 import+`__all__` 透传保公开面]+create_export 抽批量对偶拒绝闸与 dxf kwargs 组装两子函数消 PLR0915 豁免，418 行） | 导出请求 | ExportHandle/ExportMeta |
| `server/waterprint_server/services/exports_support.py` | exports 纯函数与常量支撑（ENG7 P3a：命名分量白名单/确定性命名/unit 严格化/边车文本/批量 IPC items 五纯函数+ExportMeta+kind 三常量+InvalidExportRequestError[命名闸 raise 面依赖随迁防循环 import]——零 IO 零全局态，exports 透传再导出，172 行） | 文件名分量/导出请求面（str·Mapping） | 确定性文件名/边车文本/批量载荷 |
