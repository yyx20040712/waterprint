...
不在项目装配图内的单元 id 同样显式拒绝。webapp 枚举下拉对**无档位
参数的单元置灰**（标注「无档位参数」——不可枚举），并支持中文名/
代码搜索（C2-visual 批 F6）；任务失败的错误类型以中文提示显示
（如「装配失败」——原始错误码括注保留追溯）。
...
- **页签行与留白**（C2-visual 批 2026-09-11）：顶部页签行为浅蓝面板
  底（#12213a——与单元库面板同色，方案 A 设计稿逐 token 对齐）+底部
  细边框；内容类标签（方案浏览/三维视图/高程纵断/图纸预览/概算）内容
  与单元库之间有 16px 留白呼吸位；工艺画布与厂区布置为编辑器面满铺
  呈现。
...
- **中文列名**：固定列 margin_min/nan_flag/condition_key 显示为
  ...
  **计算输出列同中文化**（V2 批 2026-09-12）：枚举结果中的计算派生
  输出量（容积/HRT/需氧量等）列头显示单元声明的中文名+单位（如
  「好氧区容积 m³」——真源=各单元 manifest 的 out_dims 声明面；
  未声明单元降级显示字段代码名，悬浮可见原名）；
- **可行性列**：每行「可行」（绿）或「不可行」（红）——标注该组参数
  能否算出完整结果；「最小裕度」列当前在全部单元上为空（内核裕度
...
- **节点 3D 缩略图**（C2-thumb 批 2026-09-11+C2-visual 迭代+C2VD 帽盖）：
  卡片图标槽显示该构筑物三维构型（**纵向半剖视角**——近半剖开露出
...
C2-ALIGN A5r 批 2026-09-12 双分页升位）：

- **双分页**（C2-ALIGN A5r）：构筑物标题（如「AAO 生物池」）下方
  两个切换页签——**约束参数**（该构筑物的设计参数面，即下述表单行）
  与**经验取值**（全项目通用的 22 条设计假设）；两页草稿独立保留
  （切换不丢已输入值）；未选中构筑物时约束页显示引导文案，经验
  取值页始终可用；
- **全部参数显示物理意义**：约束参数与经验取值均显中文名（如
  「安全超高」「池壁厚度（概算）」）；代码键悬浮可见（追溯链保留）；
- **经验取值行**：格式与约束参数行一致（中文标签+输入控件+单位
  后缀），行首多一个 ▸ 展开钮——展开显示**默认取值与出处**小字
  （规范/手册引用），被项目覆盖过的条目展开后可一键「恢复默认」；
  面板底部「提交修改」=保存全部假设修改并自动重算；
- **单行布局**：每个参数一行（左中文名+右输入控件对齐）——替代
  原四行堆叠，一屏可见参数数翻倍；**代码键（field_id）默认隐藏**
  （悬浮参数名可见——追溯链保留）；
- **单位入控件**：输入框右侧直接显示单位符号（m³/d、mg/L 等——
  C2VD 批迁移 Space.Compact 紧凑组合，控件与单位浑然一体）；
  量纲/默认值/范围/档位全量收进**控件悬浮提示**；
- **档位快选**：档位参数（如滤池格数）提供可点选标签行——点击
  即回填，当前值高亮；自由输入仍开放；
- **浮点显示归一**（C2-visual 批 F8）：参数值与输入经 12 位有效
  数字归一——二进制浮点尾差（如 0.009999999999999998）不再出现，
  显示/回填/步进三通道统一；
...
- **图标行**：每个单元一行——域色图标 22×22（与画布节点同语言：
  市政蓝/污泥棕/矿井绿/输送灰）+中文名 13px（C2-ALIGN 批 2026-09-12
  增大）；**英文代码不显示**（悬浮行可见完整 unit_id——唯一保留的
  追溯通道）；详情抽屉参数表亦显中文名（代码键悬浮可见）；
...
### 3.12 三维视图（webapp，C2-3d 批 2026-09-10）

三维视图按工程软件厂区参考系重制（无头断言 8/8 实证）：

- **地面与参考网格**：深蓝地面+双层工程网格（主格 10m/次格 2m）
  ——GPS-X 厂区参考系惯例；远缘雾化淡出，画布底色由纯黑改深蓝
  （消除「黑背景无边界融合」观感）；
- **光影**：构筑物投射落地阴影（方向光阴影相机随场景尺寸自适应）
  ——增强落地感与高程判读；
- **构筑物标签**：中文名标注（与单元库/画布同源），默认开启，
  按单元分组一条、亮色描边深底可读；
- **管线连接**：工艺连线以**高架管廊**呈现——水线蓝/泥线棕两色
  制（任一污泥端归泥色），管架高于全厂最高构筑物、管端止于结构
  边缘；当前项目未使用的单元间无管线（诚实呈现）；
- **取景**：三预设视角（等轴/俯视/侧视）+自由漫游（拖拽旋转/滚轮
  缩放/右键平移）；预设机位随场景尺寸自适应（更近更俯的默认构图）；
- **摆放态提示**（C2-visual 批 F9）：当厂区布置进行中（已摆放部分
...
- **锁定基准**：勾选工况键集→「锁定基准」→存入项目 view 态
  ...
  集变更后失效工况键灰显「（已移除）」；
- **工况校核双页**：工况对比标签顶部双页切换「对比矩阵｜工况校核」
  （与工艺画布参数面板同款制式）——校核页勾选受检单元（n-1 池检修
  敏感性）即保存（标题行直出「受检 k 单元 → 计算工况 2+k 档」代价
  提示；勾选→保存→回矩阵页看新增工况列，同一面板闭环）。注意：勾选
  受检单元需相应单元包声明检修降级映射（当前版本 13 个市政单元尚未
  声明——勾选后提交计算会被明确拒绝并提示，属数据面待补全的已知
  前置）；
- **工况键中文化**：全 UI 面工况键显示工程全称（design→「最高日最高
  时」、avg→「平均日」、design_offline_×××→「×××检修」——对比矩阵
  列头/锁定基准勾选/高程与概算工况下拉/方案表工况条件列/可信度各表
  工况列），鼠标悬浮显示原始键（对照接口文档追溯）；
