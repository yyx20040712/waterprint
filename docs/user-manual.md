...
> 事实口径声明：本手册只描述**已实现**的行为（CLI 子命令集 v2 与 API——端点计数以 docs/status.md 为准，
...
- **锁定基准**：勾选工况键集→「锁定基准」→存入项目 view 态
  ...
  集变更后失效工况键灰显「（已移除）」；
- **工况校核双页**：工况对比标签顶部双页切换「对比矩阵｜工况校核」（与
  工艺画布参数面板同款制式）——校核页勾选受检单元（n-1 池检修）即保存
  （「受检 k 单元→计算工况 2+k 档」；勾选→保存→回矩阵页看新增工况列）。
  注意：勾选需单元包声明检修降级映射（13 个市政单元未声明——勾选后
  提交计算会被拒并提示，数据面待补全前置）；
- **工况键中文化**：全 UI 面工况键显示工程全称（design→「最高日最高时」/
  avg→「平均日」/design_offline_×××→「×××检修」——矩阵列头、锁定勾选、
...
### 5.2 API（操作计数以 docs/status.md 为准，openapi 锁定断言恒）
