...
画布左侧参数面板按工程软件表单惯例重制（无头断言 12/12 实证）：

- **单行布局**：每个参数一行（左中文名+右输入控件对齐）——替代
  原四行堆叠，一屏可见参数数翻倍；**代码键（field_id）默认隐藏**
  （悬浮参数名可见——追溯链保留）；
- **单位入控件**：输入框右侧直接显示单位符号（m³/d、mg/L 等）；
  量纲/默认值/范围/档位全量收进**控件悬浮提示**；
- **档位快选**：档位参数（如滤池格数）提供可点选标签行——点击
  即回填，当前值高亮；自由输入仍开放；
- **底部常驻**：提交重算与重置按钮固定在面板底部（长参数列表滚动
  时不再滚出视口）；**重置**=一键清空全部未提交修改；
- **面板宽度可拖拽**：拖动面板右缘竖条可调整宽度（240~480px，
  默认 280px）；
- 「◈ 可行域」引导入口不变（见 §3.6）。

### 3.11 单元库（webapp，C2-lib 批 2026-09-10）

左侧单元库按画布同款象形语言重制（无头断言 12/12 实证）：

- **图标行**：每个单元一行——域色图标（与画布节点同语言：市政蓝/
  污泥棕/矿井绿/输送灰）+中文名；**英文代码不显示**（悬浮行可
  见完整 unit_id——唯一保留的追溯通道）；
- **分组浏览**：四条业务线分组（组标题带域色短条+单元计数）+
  内置节点组排末；默认全展开；底部计数条左右分列（左「32 单元」
  右「5 组」——内置节点不计入单元数）；
- **搜索**：按中文名或 unit_id 子串过滤（大小写不敏感）；
- **选中联动**：点击单元行打开详情抽屉，同时画布中该类节点亮起
  水蓝光环（定位语义——与节点点击选中的鎏金描边并存可分）；关闭
  抽屉即解除；当前项目未使用的单元画布无变化；
- **详情抽屉**：图标+名称+所属线+参数面（默认值/范围/档位）与
  端口面全量表格；「到工艺画布编辑参数」一键跳转画布标签。
