...
### 批 2 终裁定案（2026-09-18，B2-3——三段通道第 3 段收口）

三段通道走完（调研→拟定→对抗审核 B0/W12/N6/PASS→主控终裁）。终裁前对
设计书关键主张独立复核（机制件定性/429 条目=32 manifest+2 L3/22 键七域/
版本面序列化实证等——复核记录=定案件附录 D），两处前提出入经实证裁定：

1. **步②前提出入**：「formulas.py 按线分片」字面对象不存在——429 公式
   条目本就分布于 32 单元 manifest+2 L3 件（比线更细的按线分布），
   formulas.py 本体=机制件（487 行零实登记）。裁定取**意图读法**：机制件
   拆子包+条目原位；字面读法（条目抽到按线文件）违 manifest 唯一真源+
   单元包互不 import 两铁律，否决。涉裁决③字面释义，列 Rulings 呈用户
   追认（不阻塞实施）。
2. **步③条件句走假分支**：dimension_specs.py 经实证=结果字段量纲数据件
   非逐公式平行表，GR-42（量纲真源单归 FormulaSpec.output_dim）已满足
   ——步③降为验证型零动作步（三步=两实装+一确认）。列 Rulings 呈报知悉。

另经实证闭卷：D3 不引入独立 data_version（ReproTriple 序列化必填键集确含
data_version 域，B 案永久否决）。**定案全文与实装蓝图（B2-4/5/6 依据）=
`docs/design/2026-09-18_registry-split-design.md`**（审核 W12/N6 逐条处置
在彼处处置表，本块不复制防双源）。
