...
> unit_prices **1.0.0**（price_data_version 实名键——COST2 结构修正）。

| 包 | 当前状态 | 首个真实版本归属 |
|----|----------|------------------|
| unit_prices | 0.0.0（空，待迁移 2019 黑龙江定额并抽验 10%）→**现 1.0.0**（COST1 三 YAML 81 条迁移+RATIFY3 批准放行 2026-08-28——数据策略 v2 全程；结构修正 data_version→price_data_version 不升版注记；详见包内 manifest 变更注释） | M0 数据迁移 |
